...
export default function Dashboard() {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      // 2. Strict Light & Sound Filter
      const lightEnquiries = allEnquiries.filter((e) => {
        const mTitle = String(e.moduleId?.title || '').toLowerCase();
        const eType = String(e.eventType || '').toLowerCase();
        
        return mTitle.includes('light') || 
      ...
      lightEnquiries.forEach((e) => {
        ...
        const nameKey = e.moduleId?.title || e.eventType || 'System Service';
      ...
      setDashboardData({
        ...
        enquiries: lightEnquiries,
        popularEnquiries,
        monthlyGrowthEnquiries
      });
    } catch (err) {
      console.error('Light & Sound Dashboard data fetch error:', err);
    } finally {
      setLoading(false);
    }
  }, []);
      ...
      <Grid size={12}>
        <Grid container spacing={gridSpacing}>
          <Grid size={{ xs: 12, sm: 6, md: 4 }}>
            <LightEarningCard isLoading={isLoading} totalEnquiryMonth={dashboardData.monthlyEnquiries} />
          </Grid>
          <Grid size={{ xs: 12, sm: 6, md: 4 }}>
            <LightOrderchart 
              isLoading={isLoading} 
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <Grid container spacing={gridSpacing}>
              <Grid size={{ xs: 12, sm: 6, md: 12 }}>
                <LightIncomeDarkCard isLoading={isLoading} totalEnquiries={dashboardData.totalEnquiries} />
              </Grid>
              <Grid size={{ xs: 12, sm: 6, md: 12 }}>
