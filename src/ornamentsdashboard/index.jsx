...
import IncomeLightCard from './cards/OrnamentsIncomeLightCard';
import IncomeDarkCard from './cards/OrnamentsIncomeDarkCard';
import GrowthBarChart from './OrnamentsGrowthBarChart';
...
export default function OrnamentsDashboard() {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      // 2. Loose Ornaments Filter
      const ornamentsBookings = allBookings.filter((b) => {
        ...
        return mType.includes('ornament') || 
               sType.includes('ornament') || 
               pName.includes('ornament') || 
               b.ornamentId || 
               b.ornamentPackageId ||
               (b.items && b.items.some(item => String(item.name || '').toLowerCase().includes('ornament'))) ||
               (mType === '' && !b.vehicleId && !b.photographyId && !b.makeupId && !b.cakeId); // Fallback: if no clear moduleType and not explicitly another module
      });
      ...
      ornamentsBookings.forEach((b) => {
        ...
        // Packages 
        const pkgId = b.ornamentId || b.ornamentPackageId || b.packageId || b.packageName || 'unknown';
        const name = b.packageName || (typeof b.ornamentId === 'object' ? b.ornamentId?.name : null) || 'Ornaments Service';
      ...
      setDashboardData({
        totalEarnings,
        totalOrders: ornamentsBookings.length,
        totalIncome,
      ...
      console.error('Error fetching Ornaments Dashboard data:', err);
    } finally {
      setLoading(false);
    }
  }, []);
      ...
      <Grid item size={12}>
        <Grid container spacing={gridSpacing}>
          <Grid size={{ xs: 12, sm: 6, md: 4 }}>
            <OrnamentsEarningCard isLoading={isLoading} totalEarnings={dashboardData.totalEarnings} />
          </Grid>
            ...
            <OrnamentsOrderChart 
              ...
              monthChart={{
                ...
                options: {
                    ...
                    tooltip: { fixed: { enabled: false }, x: { show: false }, y: { title: { formatter: () => 'Orders' } }, marker: { show: false } }
                },
      ...
      <Grid item size={12}>
        <Grid container spacing={gridSpacing}>
          <Grid size={{ xs: 12, md: 8 }}>
            <GrowthBarChart
              isLoading={isLoading}
              title="Ornaments Growth Performance"
