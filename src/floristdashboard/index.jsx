...
export default function Dashboard() {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      // 2. Strict Florist Filter (Only pure matching Florist features to prevent bleed)
      const floristBookings = allBookings.filter((b) => {
        ...
        return mType.includes('florist') || 
               mType.includes('stage') ||
               sType.includes('florist') || 
               sType.includes('stage') ||
               pName.includes('florist') || 
               pName.includes('stage') ||
               b.floristId || 
               b.floristPackageId ||
               b.stageId ||
               (b.items && b.items.some(item => 
                 String(item.name || '').toLowerCase().includes('florist') || 
                 String(item.name || '').toLowerCase().includes('stage')
               ));
      });
      ...
      let packageNameMap = {};
      try {
        const pkgRes = await axios.get(`${API_BASE_URL}/api/florists/provider/${providerId}`, config);
        const pkgs = pkgRes.data?.data || pkgRes.data || [];
        if (Array.isArray(pkgs)) {
          pkgs.forEach((p) => {
            packageNameMap[p._id] = {
              name: p.name || p.title || p.packageName || 'Florist Service',
              price: p.price || p.pricing?.grandTotal || p.salePrice || 0
            };
          });
        }
      } catch (e) {
        console.warn('Florist list fetch failed:', e);
      }
      ...
      floristBookings.forEach((b) => {
        ...
        const pkgIdStr = typeof b.packageId === 'object' ? b.packageId?._id : (b.packageId || b.floristId || b.floristPackageId);
        ...
        if (!packageMap[pkgId]) {
          ...
          packageMap[pkgId] = {
            id: pkgId,
            name: nameStr || info?.name || 'Custom Florist Service',
      ...
      setDashboardData({
        totalEarnings,
        totalOrders: floristBookings.length, 
        totalIncome,
        monthlyOrders: monthlyOrdersCount,
        yearlyOrders: yearlyOrdersCount,
        bookings: floristBookings,
        popularPackages,
        monthlyGrowth,
        monthlyIncomeGrowth,
        totalPackageRevenue
      ...
      console.error('Florist Dashboard data fetch error:', err);
    } finally {
      setLoading(false);
    }
  }, []);
      ...
      <Grid size={12}>
        <Grid container spacing={gridSpacing}>
          <Grid size={{ xs: 12, sm: 6, md: 4 }}>
            <FloristEarningCard isLoading={isLoading} totalEarnings={dashboardData.totalEarnings} />
          </Grid>
          <Grid size={{ xs: 12, sm: 6, md: 4 }}>
            <FloristOrderchart 
              isLoading={isLoading} 
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <Grid container spacing={gridSpacing}>
              <Grid size={{ xs: 12, sm: 6, md: 12 }}>
                <FloristIncomeDarkCard isLoading={isLoading} totalAmount={dashboardData.totalIncome} />
              </Grid>
              <Grid size={{ xs: 12, sm: 6, md: 12 }}>
