...
// chart data
import barChartOptions from '../vehicledashboard/chartdata/vehicle-growth-barchart';
...
export default function VenueTotalGrowthBarChart({ 
  isLoading, 
  monthlyGrowth = [], 
  monthlyIncomeGrowth = [], 
  totalEarnings = 0 
  ...
  // Filter Data based on selected period - Only showing income as per user request
  const chartData = useMemo(() => {
    if (period === 'month') {
      return {
        series: [{ name: 'Revenue (₹)', data: [monthlyIncomeGrowth[currentMonthIdx] || 0] }],
  ...
  }, [period, monthlyIncomeGrowth, currentMonthIdx]);
...
VenueTotalGrowthBarChart.propTypes = { 
