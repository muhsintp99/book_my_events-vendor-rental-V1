...
const BoutiqueBookings = ({ initialTab = 'All', hideTabs = false }) => {
    ...
    const tabs = useMemo(() => [
        ...
        { label: 'Confirmed', value: 2, filterFn: (s) => ['accepted', 'confirmed', 'approve', 'approved'].includes(s), key: 'Confirmed' },
        { label: 'Completed', value: 3, filterFn: (s) => s === 'completed', key: 'Completed' },
        { label: 'Cancelled', value: 4, filterFn: (s) => ['cancelled', 'rejected'].includes(s), key: 'Cancelled' },
        { label: 'Payment Failed', value: 5, filterFn: (s, p) => p === 'failed' || p === 'Payment Failed', key: 'Payment Failed' },
    ], []);
    ...
    const fetchAllBookings = async (pid) => {
        ...
        try {
            ...
            const boutiqueBookings = all.filter(b => {
                const mType = String(b.moduleType || '').toLowerCase();
                return mType === 'boutique' || mType === 'boutique artist';
            });
            setBookings(boutiqueBookings);
        } catch (err) {
    ...
    const processedBookings = useMemo(() => {
        ...
        if (currentTab && (currentTab.label !== 'All' || hideTabs)) {
            result = result.filter(b => {
                ...
                return currentTab.filterFn(status, payStatus);
            });
        }
        ...
        if (filters.packageType !== 'all') {
            result = result.filter(b => {
                const pkg = getBoutiquePackageData(b);
                return pkg.packageType.toLowerCase() === filters.packageType.toLowerCase();
            });
        }
        ...
        <Box sx={{ minHeight: '100vh', bgcolor: '#f8fafc', pb: 6 }}>
                ...
                <Paper elevation={0} sx={{ borderRadius: '16px', border: '1px solid #e2e8f0', overflow: 'hidden', bgcolor: '#fff' }}>
                    ...
                    <Box sx={{ p: 3, borderBottom: '1px solid #f1f5f9' }}>
                        <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} justifyContent="space-between" alignItems={{ xs: 'stretch', md: 'center' }}>
                            <Typography variant="h6" fontWeight={800} sx={{ color: '#1e293b' }}>My Bookings</Typography>
                    ...
                    {!isMobile && (
                        <Box sx={{ px: 3, py: 2, bgcolor: '#f8fafc', borderBottom: '1px solid #f1f5f9', display: 'flex', gap: 2, alignItems: 'center' }}>
                            ...
                            <TableLabel flex={2.5} label="Package" />
                            ...
                            <TableLabel flex={1.5} label="Payment Status" />
            ...
            >
                <Box sx={{ px: 2, py: 1.5, borderBottom: '1px solid #f1f5f9' }}>
                    <Typography variant="caption" fontWeight={700} color="text.secondary" sx={{ textTransform: 'uppercase', letterSpacing: 1 }}>Sort By</Typography>
                </Box>
                {[
                    { value: 'newest', label: 'Latest First', icon: <ArrowDownward fontSize="small" /> },
                    { value: 'date_desc', label: 'Event Date (Newest First)', icon: <ArrowDownward fontSize="small" /> },
                    { value: 'date_asc', label: 'Event Date (Oldest First)', icon: <ArrowUpward fontSize="small" /> },
                    ...
                    { value: 'name_asc', label: 'Customer (A-Z)', icon: <ArrowUpward fontSize="small" /> },
                    { value: 'name_desc', label: 'Customer (Z-A)', icon: <ArrowDownward fontSize="small" /> }
                ].map((option) => (
                    <MenuItem
                        key={option.value}
                        ...
                        sx={{
                            ...
                            '&.Mui-selected': {
                                ...
                                color: GOLD_COLOR,
                                fontWeight: 700,
                                '&:hover': { bgcolor: alpha(GOLD_COLOR, 0.15) }
                            }
                        }}
                    >
                        {option.icon}
                        <Typography variant="body2">{option.label}</Typography>
                        {sortBy === option.value && <CheckCircle sx={{ ml: 'auto', fontSize: 18 }} />}
                    </MenuItem>
                ))}
            </Menu>
        </Box>
...
const BookingRow = ({ index, booking, isMobile, onUpdateStatus, onView, statusColor }) => {
    ...
    const boutiquePackage = getBoutiquePackageData(booking);

    if (isMobile) {
        return (
            <Box sx={{ p: 2, borderBottom: '1px solid #f1f5f9', cursor: 'pointer' }} onClick={() => onView(booking)}>
                <Stack direction="row" spacing={1.5} alignItems="center">
                    <Avatar
                        src={getImageUrl(boutiquePackage.thumbnail)}
                        variant="rounded"
                    ...
                    <Box flex={1}>
                        ...
                        <Typography variant="subtitle2" fontWeight={800} noWrap sx={{ maxWidth: '60vw' }}>{boutiquePackage.name}</Typography>
                        <Chip label={boutiquePackage.packageType} size="small" sx={{ height: 16, fontSize: '0.6rem', fontWeight: 700 }} />
                ...
                <Stack direction="row" justifyContent="space-between" mt={1.5} alignItems="center">
                    ...
                    <Stack direction="row" spacing={1}>
                        <IconButton size="small" sx={{ border: '1px solid #e2e8f0', color: GOLD_COLOR }} onClick={(e) => { e.stopPropagation(); window.open(`tel:${booking.contactNumber}`); }}><Phone fontSize="inherit" /></IconButton>
                        <IconButton size="small" sx={{ border: '1px solid #e2e8f0', color: '#25d366' }} onClick={(e) => { e.stopPropagation(); window.open(`https://wa.me/${booking.contactNumber}`); }}><WhatsApp fontSize="inherit" /></IconButton>
                    </Stack>
                </Stack>
            </Box>
        );
    ...
    return (
        <Box sx={{
            ...
            <Box sx={{ flex: 2.5, display: 'flex', alignItems: 'center', gap: 1.5 }}>
                ...
                />
                <Box>
                    <Typography variant="body2" fontWeight={700} noWrap sx={{ maxWidth: 150 }}>{boutiquePackage.name}</Typography>
                    <Stack direction="row" spacing={0.5} alignItems="center">
                        <Chip label={boutiquePackage.packageType} size="small" sx={{ height: 16, fontSize: '0.55rem', fontWeight: 700 }} />
                        <Typography variant="caption" color="text.secondary">{boutiquePackage.category}</Typography>
                    </Stack>
                </Box>
            </Box>

            <Typography variant="body2" sx={{ flex: 2, fontWeight: 700 }}>{booking.fullName}</Typography>
            <Typography variant="body2" sx={{ flex: 2.5, color: 'text.secondary', fontSize: '0.8rem' }}>{booking.emailAddress}</Typography>
            <Typography variant="body2" sx={{ flex: 1.5 }}>{new Date(booking.bookingDate).toLocaleDateString()}</Typography>

            <Typography variant="body2" sx={{ flex: 1.5, fontWeight: 800, color: '#0f172a' }}>
                ₹{parseFloat(booking.finalPrice || 0).toLocaleString()}
            </Typography>
            <Box sx={{ flex: 1.5 }}>
                <Typography variant="caption" fontWeight={800} sx={{ px: 1, py: 0.5, borderRadius: '6px', color: statusColor, bgcolor: alpha(statusColor, 0.1) }}>
                    {booking.status}
                </Typography>
            </Box>
            <Box sx={{ flex: 1.5 }}>
                <Typography variant="caption" fontWeight={700} color={booking.paymentStatus === 'Initiated' ? '#f59e0b' : 'text.primary'}>
            ...
            <Stack direction="row" spacing={1} sx={{ flex: 2 }} justifyContent="center">
                {isPending ? (
                    <>
                        <Tooltip title="Confirm Booking">
                            <IconButton
                                size="small"
                                color="success"
                                onClick={(e) => onUpdateStatus(booking._id, 'accepted', e)}
                                sx={{ border: '1px solid', borderColor: 'success.light', bgcolor: alpha('#22c55e', 0.05) }}
                            >
                        ...
                        <Tooltip title="Reject Booking">
                            <IconButton
                                size="small"
                                color="error"
                                onClick={(e) => onUpdateStatus(booking._id, 'rejected', e)}
                                sx={{ border: '1px solid', borderColor: 'error.light', bgcolor: alpha('#ef4444', 0.05) }}
                            >
                    ...
                    <>
                        <Button onClick={(e) => onView(booking, e)} size="small" variant="outlined" sx={{ minWidth: 50, borderRadius: '6px', textTransform: 'none', fontSize: '0.7rem' }}>View</Button>
                        <Button size="small" variant="outlined" sx={{ minWidth: 70, borderRadius: '6px', textTransform: 'none', fontSize: '0.7rem' }}>Download</Button>
                    </>
                )}
            </Stack>
        </Box>
...
// Simple Detail Row for clean layout
const DetailRow = ({ icon, label, value }) => (
    <Stack direction="row" spacing={1.5} alignItems="flex-start">
        <Box sx={{ color: 'text.secondary', mt: 0.3 }}>{icon}</Box>
        <Box>
            <Typography variant="caption" display="block" color="text.secondary" fontWeight={500} lineHeight={1}>
                {label}
            </Typography>
...
// Helper to reliably render time slots (copied from existing code)
...
/* ----------------------------------------------------------------------------------
   SUB-COMPONENT: Premium Detail Drawer
---------------------------------------------------------------------------------- */
            ...
            <Stack spacing={4}>
                ...
                <Box sx={{ p: 3, borderRadius: '20px', bgcolor: alpha(statusColor, 0.05), border: '1px solid', borderColor: alpha(statusColor, 0.1) }}>
                    <Stack direction="row" justifyContent="space-between" alignItems="center">
                        ...
                        <Chip label={booking.bookingType || 'Rental'} sx={{ fontWeight: 700, bgcolor: '#fff' }} />
                    </Stack>
                </Box>
                    ...
                    <Stack direction="row" spacing={3} mt={2} alignItems="center">
                        <Box
                            sx={{
                                ...
                                borderRadius: '16px',
                                overflow: 'hidden',
                                border: `2px solid ${alpha(GOLD_COLOR, 0.2)}`,
                                boxShadow: '0 8px 16px rgba(0,0,0,0.05)'
                            }}
                        >
                            <img
                                src={getImageUrl(boutiquePackage.thumbnail)}
                                alt={boutiquePackage.name}
                                style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                                onError={(e) => { e.target.src = 'https://via.placeholder.com/100?text=Package'; }}
                            />
                        </Box>
                        <Box flex={1}>
                            <Typography variant="h6" fontWeight={800} color={GOLD_COLOR}>{boutiquePackage.name}</Typography>
                            <Grid container spacing={1} mt={0.5}>
                                <Grid item xs={6}>
                                    <DetailRow icon={<Category fontSize="small" />} label="Type" value={boutiquePackage.packageType} />
                                </Grid>
                                <Grid item xs={6}>
                                    <DetailRow icon={<Category fontSize="small" />} label="Category" value={boutiquePackage.category} />
                                </Grid>
                                {boutiquePackage.sizes && boutiquePackage.sizes.length > 0 && (
                                    <Grid item xs={12}>
                                        <Typography variant="caption" color="text.secondary" display="block">Available Sizes</Typography>
                                        <Stack direction="row" spacing={0.5} mt={0.5} flexWrap="wrap">
                                            {boutiquePackage.sizes.map((size, idx) => (
                                                <Chip key={idx} label={size} size="small" sx={{ height: 20, fontSize: '0.65rem' }} />
                                            ))}
                                        </Stack>
                                    </Grid>
                                )}
                                {boutiquePackage.colors && boutiquePackage.colors.length > 0 && (
                                    <Grid item xs={12}>
                                        <Typography variant="caption" color="text.secondary" display="block">Available Colors</Typography>
                                        <Stack direction="row" spacing={0.5} mt={0.5} flexWrap="wrap">
                                            {boutiquePackage.colors.map((color, idx) => (
                                                <Chip key={idx} label={color} size="small" sx={{ height: 20, fontSize: '0.65rem' }} />
                                            ))}
                                        </Stack>
                                    </Grid>
                                )}
                ...
                <Box>
                    <SectionLabel label="Customer Profiles" />
                    <Stack spacing={2} sx={{ mt: 2 }}>
                        <DetailRow icon={<Person />} label="Full Name" value={booking.fullName} />
                        <DetailRow icon={<Email />} label="Email Address" value={booking.emailAddress} />
                        <DetailRow icon={<Phone />} label="Phone Number" value={booking.contactNumber} />
                    </Stack>
                </Box>
                ...
                <Box>
                    ...
                    <Grid container spacing={2} sx={{ mt: 1 }}>
                        <Grid item xs={6}><DetailRow icon={<CalendarMonth />} label="Event Date" value={new Date(booking.bookingDate).toLocaleDateString()} /></Grid>
                        <Grid item xs={6}><DetailRow icon={<AccessTime />} label="Time Slot" value={renderTimeSlot(booking.timeSlot)} /></Grid>
                        <Grid item xs={6}><DetailRow icon={<EventAvailable />} label="Package Type" value={booking.status === 'confirmed' ? 'Reserved' : 'Inquiry'} /></Grid>
                    </Grid>
                </Box>

                <Divider sx={{ borderStyle: 'dotted' }} />

                <Box sx={{ p: 3, borderRadius: '24px', background: PREMIUM_DARK, color: '#fff', boxShadow: '0 12px 24px rgba(0,0,0,0.2)' }}>
                    <Stack direction="row" justifyContent="space-between" alignItems="center">
                        <Box>
                            <Typography variant="caption" sx={{ opacity: 0.6, fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: 1 }}>Grand Total</Typography>
                            <Typography variant="h4" fontWeight={900} color={GOLD_COLOR}>₹{parseFloat(booking.finalPrice || 0).toLocaleString()}</Typography>
                        </Box>
                        <Chip
                            label={booking.paymentStatus || 'Initiated'}
                            sx={{
                                fontWeight: 800,
                                bgcolor: booking.paymentStatus === 'Completed' ? 'success.main' : alpha(GOLD_COLOR, 0.2),
                                color: booking.paymentStatus === 'Completed' ? '#fff' : GOLD_COLOR,
                                borderRadius: '10px'
                            }}
                        />
                    </Stack>
                ...
                <Stack direction="row" spacing={2}>
                    {String(booking.status).toLowerCase() === 'pending' && (
                        <>
                            <Button fullWidth variant="outlined" color="error" onClick={(e) => { onUpdateStatus(booking._id, 'rejected', e); onClose(); }} sx={{ borderRadius: '12px', height: 48 }}>Reject</Button>
                            <Button fullWidth variant="contained" color="success" onClick={(e) => { onUpdateStatus(booking._id, 'accepted', e); onClose(); }} sx={{ borderRadius: '12px', height: 48 }}>Confirm</Button>
                        </>
                    )}
                    <Button fullWidth variant="soft" startIcon={<WhatsApp />} sx={{ borderRadius: '12px', height: 48, bgcolor: alpha('#25d366', 0.1), color: '#25d366' }} onClick={() => window.open(`https://wa.me/${booking.contactNumber}`, '_blank')}>WhatsApp</Button>
                </Stack>
            </Stack>
        </Drawer>
    );
