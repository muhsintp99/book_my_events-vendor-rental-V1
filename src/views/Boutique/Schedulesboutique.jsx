...
function BookingCalendar() {
  ...
  const fetchBoutiquePackages = async () => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/boutiques/provider/${providerId}`
      );
      const data = await response.json();
      ...
      console.error('Fetch boutique packages error:', err);
      setPackages([]);
    }
  };
              ...
              >
                ...
                <Box sx={{
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    ...
                    <Box>
                      <Typography sx={{ fontSize: '11px', color: '#999', textTransform: 'uppercase', fontWeight: '600' }}>Type</Typography>
                      <Typography sx={{ fontSize: '14px', fontWeight: '500' }}>{booking.bookingType} Appointment</Typography>
                    </Box>
                  </Box>
                </Box>
              </Box>
