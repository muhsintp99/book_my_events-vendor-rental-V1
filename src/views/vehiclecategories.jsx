...
const VehicleCategory = () => {
  ...
  const moduleId = localStorage.getItem('moduleId'); // Get vehicle module ID from localStorage
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      try {
        ...
        if (!moduleId) {
          throw new Error('Module ID not found. Please login again.');
        }

        // Fetch parent categories for vehicle module
        const parentResponse = await fetch(`${API_BASE_URL}/api/categories/parents/${moduleId}`, {
          headers: {
        ...
        if (parentData.success && Array.isArray(parentData.data)) {
          ...
          setError('No categories found for vehicle module');
        }
      } catch (error) {
  ...
  const handleExport = (format) => {
    ...
    link.href = url;
    link.download = `vehicle_categories_${new Date().toISOString().slice(0, 10)}.${format}`;
    link.click();
                      ...
                      <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={5}>
                        <Collapse in={expandedRows[category.id]} timeout="auto" unmountOnExit>
                          <Box sx={{ margin: 2, bgcolor: '#fafafa', borderRadius: 1, p: 2 }}>
                            <Typography variant="subtitle2" gutterBottom sx={{ color: '#E15B65', fontWeight: 600 }}>
                              Subcategories
                            </Typography>
