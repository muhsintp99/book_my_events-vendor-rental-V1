...
import { Box, Typography, TextField, Button, Card, CardContent, CardMedia, IconButton, Stack, Radio, FormControlLabel, Select, MenuItem, FormControl, InputLabel, Switch, Snackbar, Alert, CircularProgress, Checkbox, Dialog, DialogContent, DialogActions, RadioGroup } from '@mui/material';
...
const CreateAuditorium = () => {
  ...
  const fetchPackages = async () => {
    try {
      ...
      if (!providerId || !token) {
        console.warn('Provider ID or token not found.');
        setToast({
          open: true,
          message: 'No packages found. Please ensure providerId is set and packages are created with provider association.',
          severity: 'warning',
        });
        setFetchedPackages([]);
        return;
  ...
  const handlePackageSelect = (pkgId) => {
    setFormData(prev => ({ ...prev, selectedPackageId: pkgId }));
    setToast({ open: true, message: 'Package selected!', severity: 'success' });
  };
  ...
  const validateForm = () => {
    ...
    if (formData.foodCateringAvailability && !formData.selectedPackageId) {
      errors.push('Food package is required');
    }
    return errors;
  };
  ...
  const fetchVenue = async (id) => {
    ...
    try {
      ...
      setFormData({
        ...
        selectedPackageId: result.data.packageId || '',
      });
...
const handleSubmit = async (event) => {
  ...
  try {
    let packageId = formData.selectedPackageId;
    let venueId = id;
    ...
    delete payload.selectedPackageId;
    ...
    if (packageId) {
      ...
      data.append('packages', JSON.stringify([packageId]));
    }
                    ...
                    {formData.foodCateringAvailability && (
                      <Box sx={{ gridColumn: '1 / -1', border: `1px solid ${theme.palette.grey[300]}`, borderRadius: 1, p: 2 }}>
                        <Typography variant="subtitle1" gutterBottom sx={{ gridColumn: '1 / -1' }}>
                          Select Food & Catering Package
                        </Typography>
                        {fetchedPackages.length === 0 ? (
                          <Box sx={{ textAlign: 'center', p: 2, color: 'text.secondary' }}>
                            <Typography>No packages available. Please create packages in the Food Menu section first.</Typography>
                          </Box>
                        ) : (
                          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(1, 1fr)', sm: 'repeat(2, 1fr)' }, gap: 2 }}>
                            {fetchedPackages.map((pkg) => (
                              <Card
                                ...
                                sx={{
                                  cursor: 'pointer',
                                  border: formData.selectedPackageId === pkg._id ? `2px solid ${theme.palette.primary.main}` : '1px solid transparent',
                              ...
                              >
                                <CardMedia
                                  component="img"
                                  height="140"
                                  image={pkg.thumbnail ? `${API_BASE_URL.replace('/api', '')}/${pkg.thumbnail}` : '/placeholder.jpg'}
                                  alt={pkg.title}
                                  sx={{ objectFit: 'cover' }}
                                />
                                <CardContent sx={{ p: 2 }}>
                                  ...
                                  <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
                                    {pkg.subtitle || 'No subtitle available'}
                                  </Typography>
                                  <Typography variant="body2" color="primary">
                                    ₹{pkg.priceRange?.min || 0} - ₹{pkg.priceRange?.max || 0} per head
                                  </Typography>
                                </CardContent>
                              </Card>
                            ))}
