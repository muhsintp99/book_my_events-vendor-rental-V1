...
var ACCENT = '#FF6F00';
var ACCENT_DARK = '#E65100';
var ACCENT_LIGHT = '#FFF3E0';
var ACCENT_BG = '#FFF8F0';
            ...
            {(!isMobile || !mobileChatOpen) && (
                <Box sx={{
                    ...
                    <Box sx={{ flexGrow: 1, overflowY: 'auto' }}>
                        {loading ? (
                            <Box display="flex" justifyContent="center" py={4}><CircularProgress size={28} sx={{ color: ACCENT }} /></Box>
                        ) : filteredEnquiries.length === 0 ? (
                            <Box textAlign="center" py={4} color="text.secondary"><Typography variant="body2">No enquiries found</Typography></Box>
                        ) : (
                            <List sx={{ py: 0 }}>
                                                ...
                                                <ListItemText
                                                    ...
                                                    secondary={
                                                        <Typography variant="body2" noWrap sx={{ maxWidth: '200px', color: '#64748B', fontSize: '0.8rem' }}>
                                                            {enq.moduleId?.title || enq.email || 'Transport Enquiry'}
                                                        </Typography>
                                                    }
                                                />
                                            </ListItemButton>
                        ...
                        <React.Fragment>
                            <Box sx={{ p: isMobile ? 1.5 : 2, bgcolor: 'white', borderBottom: '1px solid #E0E4EC', display: 'flex', alignItems: 'center' }}>
                                <Stack direction="row" spacing={2} alignItems="center">
                                    {isMobile && (
                                        <IconButton onClick={function () { setMobileChatOpen(false); }}><ArrowBackIcon /></IconButton>
                                    )}
                                    ...
                                    <Box>
                                        ...
                                        <Stack direction={isMobile ? 'column' : 'row'} spacing={isMobile ? 0 : 1.5} alignItems={isMobile ? 'flex-start' : 'center'}>
                                            {activeEnquiry.email && <Typography variant="caption" sx={{ color: ACCENT, fontWeight: 700 }}>{activeEnquiry.email}</Typography>}
                                            {!isMobile && activeEnquiry.contact && <Box sx={{ width: 4, height: 4, borderRadius: '50%', bgcolor: '#CBD5E1' }} />}
                                            {activeEnquiry.contact && <Typography variant="caption" sx={{ color: '#64748B', fontWeight: 600 }}>{activeEnquiry.contact}</Typography>}
                                        </Stack>
                                    </Box>
                                </Stack>
                            </Box>
                                        ...
                                        return (
                                            <Box key={msg._id || i} sx={{
                                                ...
                                                <Paper elevation={0} sx={{
                                                    ...
                                                    bgcolor: fromMe ? ACCENT : 'white', color: fromMe ? 'white' : '#1A1A2E',
                                                    boxShadow: fromMe ? '0 4px 12px rgba(255,111,0,0.3)' : '0 2px 8px rgba(0,0,0,0.06)',
