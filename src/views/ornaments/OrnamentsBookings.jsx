...
const OrnamentsBookings = ({ initialTab = 'All', hideTabs = false }) => {
    ...
    const fetchAllBookings = async (pid) => {
        ...
        try {
            ...
            const ornamentBookings = all.filter(b => {
                const mType = String(b.moduleType || '').toLowerCase();
                return mType === 'ornament' || mType === 'ornaments';
            });
            setBookings(ornamentBookings);
        } catch (err) {
                ...
                <Paper elevation={0} sx={{ borderRadius: '16px', border: '1px solid #e2e8f0', overflow: 'hidden', bgcolor: '#fff' }}>
                    ...
                    {!isMobile && (
                        <Box sx={{ px: 3, py: 2, bgcolor: '#f8fafc', borderBottom: '1px solid #f1f5f9', display: 'flex', gap: 2, alignItems: 'center' }}>
                            ...
                            <TableLabel flex={2.5} label="Ornament" />
...
const BookingRow = ({ index, booking, isMobile, onUpdateStatus, onView, statusColor }) => {
    ...
    const ornament = getOrnamentData(booking);
        ...
        return (
            <Box sx={{ p: 2, borderBottom: '1px solid #f1f5f9', cursor: 'pointer' }} onClick={() => onView(booking)}>
                <Stack direction="row" spacing={1.5} alignItems="center">
                    <Avatar
                        src={getImageUrl(ornament.thumbnail)}
                        variant="rounded"
                    ...
                    <Box flex={1}>
                        ...
                        <Typography variant="subtitle2" fontWeight={800} noWrap sx={{ maxWidth: '60vw' }}>{ornament.name}</Typography>
    ...
    return (
        <Box sx={{
            ...
            <Box sx={{ flex: 2.5, display: 'flex', alignItems: 'center', gap: 1.5 }}>
                ...
                >
                    <Diamond sx={{ fontSize: 20, color: GOLD_COLOR }} />
                </Avatar>
                <Box>
                    <Typography variant="body2" fontWeight={700} noWrap sx={{ maxWidth: 150 }}>{ornament.name}</Typography>
                    <Typography variant="caption" color="text.secondary" sx={{ display: 'block' }}>{ornament.category}</Typography>
                </Box>
            </Box>
        ...
        <Drawer anchor="right" open={open} onClose={onClose} PaperProps={{ sx: { width: { xs: '100%', sm: 500 }, borderRadius: '24px 0 0 24px', p: 4 } }}>
            ...
            <Stack spacing={4}>
                ...
                <Box sx={{ p: 3, borderRadius: '24px', bgcolor: alpha(GOLD_COLOR, 0.03), border: '1px solid', borderColor: alpha(GOLD_COLOR, 0.1) }}>
                    <SectionLabel label="Ornament Selection" />
                    <Stack direction="row" spacing={3} mt={2} alignItems="center">
                        ...
                        >
                            <img
                                ...
                                onError={(e) => { e.target.src = 'https://via.placeholder.com/100?text=Jewel'; }}
                            />
                        </Box>
                        <Box flex={1}>
                            <Typography variant="h6" fontWeight={800} color={GOLD_COLOR}>{ornament.name}</Typography>
                            <Grid container spacing={1} mt={0.5}>
                                <Grid item xs={6}>
                                    <DetailRow icon={<Category fontSize="small" />} label="Category" value={ornament.category} />
                                </Grid>
                                <Grid item xs={6}>
                                    <DetailRow icon={<Diamond fontSize="small" />} label="SKU/ID" value={ornament.id?.slice(-8).toUpperCase()} />
                                </Grid>
                            </Grid>
                        </Box>
                    </Stack>
