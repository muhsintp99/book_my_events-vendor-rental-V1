...
function OrnamentsSchedules() {
  ...
  const resetForm = () => {
    setFormData({ fullName: '', contactNumber: '', emailAddress: '', address: '', additionalNotes: '', moduleId: modules[0]?._id || '', packageId: '', bookingDate: new Date().toISOString().split('T')[0], paymentType: 'Cash', bookingType: 'Direct', rentalType: 'Rental' });
    setActiveStep(0);
  };
            ...
            <Grid item xs={12} md={6} lg={4} key={b._id}>
              <Paper sx={{ p: 3, borderRadius: '24px', position: 'relative', overflow: 'hidden', border: '1px solid #f0f0f0', boxShadow: '0 8px 20px rgba(0,0,0,0.02)' }}>
                ...
                <Stack spacing={1}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: '#666' }}><DiamondIcon sx={{ fontSize: 16 }} /><Typography variant="body2">Ornaments</Typography></Box>
                </Stack>
              </Paper>
            </Grid>
          ))
