...
function OrnamentsSchedules() {
    ...
    try {
      ...
      if (data.success) {
        const filtered = (data.data || []).filter(b =>
          String(b.moduleType || '').toLowerCase().includes('ornament')
        );
  ...
  const fetchModules = async () => {
    try {
      ...
      const filtered = modulesList.filter(m => (m.title || m.name || '').toLowerCase().includes('ornament'));
      setModules(filtered);
  ...
  const steps = ['Customer Info', 'Order Details', 'Schedule & Notes'];
  ...
  return (
    <Box sx={{ width: '100%', p: '40px', bgcolor: '#fafafa', minHeight: '100vh' }}>
      ...
      <Box sx={{ bgcolor: '#fff', borderRadius: '24px', p: '32px', mb: '32px', border: '1px solid #f0f0f0', boxShadow: '0 4px 20px rgba(0,0,0,0.03)', display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <Box sx={{ flex: 1 }}>
          <Typography variant="h6" fontWeight={700} color="#333">{selectedBookings.length === 0 ? 'Clear Schedule' : `${selectedBookings.length} Active Orders`}</Typography>
          <Typography color="#999" fontSize="14px">{selectedBookings.length === 0 ? 'No manual bookings for this date' : 'Scheduled deliveries or rentals'}</Typography>
        </Box>
            ...
            <Grid item xs={12} md={6} lg={4} key={b._id}>
              <Paper sx={{ p: 3, borderRadius: '24px', position: 'relative', overflow: 'hidden', border: '1px solid #f0f0f0', boxShadow: '0 8px 20px rgba(0,0,0,0.02)' }}>
                ...
                <Stack direction="row" justifyContent="space-between" mb={2}>
                  <Chip label={b.bookingMode === 'rental' ? 'Rental' : 'Purchase'} size="small" icon={b.bookingMode === 'rental' ? <RentalIcon sx={{ fontSize: 14 }} /> : <PurchaseIcon sx={{ fontSize: 14 }} />} sx={{ bgcolor: 'rgba(239, 83, 80, 0.1)', color: '#ef5350', fontWeight: 700, textTransform: 'capitalize' }} />
                ...
                <Typography variant="body2" color="#666" sx={{ mb: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
                  <DiamondIcon sx={{ fontSize: 16, color: '#ef5350' }} />
                  {renderSafe(b.ornamentId?.name || 'Ornament')}
                </Typography>
                <Stack spacing={1}>
                  ...
                  {b.bookingMode === 'rental' && <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: '#666' }}><AccessTimeIcon sx={{ fontSize: 16 }} /><Typography variant="body2">{b.days || 1} Day(s)</Typography></Box>}
                </Stack>
              </Paper>
            </Grid>
          ))
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '28px', p: 1 } }}>
        <DialogTitle sx={{ position: 'relative' }}>
          ...
          <Typography variant="h4" fontWeight={800} sx={{ mt: 2, textAlign: 'center' }}>Manual Booking</Typography>
          ...
          <Box sx={{ mt: 2 }}>
            {activeStep === 0 && (
              <Fade in={activeStep === 0}><Stack spacing={3}>
                ...
                <TextField fullWidth label="Address" name="address" value={formData.address} onChange={handleInputChange} InputProps={{ startAdornment: <InputAdornment position="start"><HomeIcon sx={{ color: '#ef5350' }} /></InputAdornment> }} sx={{ '& .MuiOutlinedInput-root': { borderRadius: '16px' } }} />
              </Stack></Fade>
            )}
            {activeStep === 1 && (
              <Fade in={activeStep === 1}><Stack spacing={3}>
                <FormControl fullWidth sx={{ '& .MuiOutlinedInput-root': { borderRadius: '16px' } }}>
                  <InputLabel>Select Ornament</InputLabel>
                  <Select label="Select Ornament" name="packageId" value={formData.packageId} onChange={handleInputChange}>
                    {packages.map(pkg => (
                      <MenuItem key={pkg._id} value={pkg._id} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Typography variant="body2">{renderSafe(pkg.name)}</Typography>
                        <Chip 
                          label={pkg.availabilityMode === 'rental' ? 'Rental' : 'Purchase'} 
                          size="small" 
                          sx={{ 
                            ml: 2, 
                            height: 20, 
                            fontSize: '10px', 
                            fontWeight: 700,
                            bgcolor: pkg.availabilityMode === 'rental' ? 'rgba(33, 150, 243, 0.1)' : 'rgba(76, 175, 80, 0.1)',
                            color: pkg.availabilityMode === 'rental' ? '#2196f3' : '#4caf50'
                          }} 
                        />
                      </MenuItem>
                    ))}
                ...
                <Box>
                  <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 1, color: '#666' }}>Booking Type</Typography>
                  <ToggleButtonGroup value={formData.bookingMode} exclusive onChange={(e, val) => val && setFormData(prev => ({ ...prev, bookingMode: val }))} fullWidth sx={{ mb: 2 }}>
                    <ToggleButton value="purchase" sx={{ borderRadius: '12px 0 0 12px', py: 1.5 }}>
                      <PurchaseIcon sx={{ mr: 1 }} /> Purchase
                    </ToggleButton>
                    <ToggleButton value="rental" sx={{ borderRadius: '0 12px 12px 0', py: 1.5 }}>
                      <RentalIcon sx={{ mr: 1 }} /> Rental
                    </ToggleButton>
                  </ToggleButtonGroup>
                </Box>
                {formData.bookingMode === 'rental' && (
                  <TextField fullWidth label="Rental Duration (Days)" type="number" name="days" value={formData.days} onChange={handleInputChange} InputProps={{ startAdornment: <InputAdornment position="start"><AccessTimeIcon sx={{ color: '#ef5350' }} /></InputAdornment> }} sx={{ '& .MuiOutlinedInput-root': { borderRadius: '16px' } }} />
                )}
              </Stack></Fade>
            )}
            {activeStep === 2 && (
...
export default OrnamentsSchedules;