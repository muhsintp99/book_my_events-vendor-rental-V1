...
const MEHANDI_COLOR = '#C4572A';
const GRADIENT = 'linear-gradient(135deg, #C4572A 0%, #FF8C00 100%)';
const API_BASE = 'https://api.bookmyevent.ae';
const MEHANDI_MODULE_ID = '68e78b706a1614cf448a35a3';
...
const Multizones = () => {
  ...
  const getVendorId = () => {
    try {
      ...
      if (vendorData) {
        ...
        // Important: Using the _id that matches the mehandi vendor collection
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 2. Fetch from Mehandi-specific endpoint first (Essential for 'wayanad' zone)
      const mehandiRes = await axios.get(`${API_BASE}/api/mehandi/vendors/${MEHANDI_MODULE_ID}`);
      
      let foundVendor = null;
      if (mehandiRes.data.success && Array.isArray(mehandiRes.data.data)) {
        foundVendor = mehandiRes.data.data.find(v => v._id === vendorId || v.providerId === vendorId);
      }

      if (foundVendor) {
        setVendorProfile(foundVendor);
        const initialSelected = [];
        
        // Extract Primary Zone from singular 'zone' object
        if (foundVendor.zone) {
          const zId = typeof foundVendor.zone === 'object' ? foundVendor.zone._id : foundVendor.zone;
          if (zId) initialSelected.push(zId);
        }
        
        // Extract Multi Zones from 'zones' array
        if (foundVendor.zones && Array.isArray(foundVendor.zones)) {
          foundVendor.zones.forEach(z => {
            const zId = typeof z === 'object' ? z._id : z;
            if (zId && !initialSelected.includes(zId)) {
              initialSelected.push(zId);
            }
          });
        }
        setSelectedZones(initialSelected);
      } else {
        // Fallback to vendorprofiles
        const profileRes = await axios.get(`${API_BASE}/api/vendorprofiles/find/${vendorId}?moduleId=${MEHANDI_MODULE_ID}`);
        if (profileRes.data.success && profileRes.data.data) {
          ...
          setVendorProfile({ providerId: vendorId, module: MEHANDI_MODULE_ID });
        }
      }
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let res;
      // If it's a Mehandi record, we MUST update using the mehandi vendor schema
      const isMehandiArtistRecord = vendorProfile.firstName !== undefined;
      
      if (isMehandiArtistRecord) {
        const primaryZone = selectedZones.length > 0 ? selectedZones[0] : null;
        const additionalZones = selectedZones.slice(1);
        
        res = await axios.put(`${API_BASE}/api/mehandi/vendors/${vendorProfile._id}`, {
          zone: primaryZone,
          zones: additionalZones
        });
      } else {
  ...
  const selectedZoneObjects = selectedZones
    .map(id => {
      ...
      if (!zone && vendorProfile?.zone && (vendorProfile.zone._id === id || vendorProfile.zone === id)) {
  ...
  if (loading) {
    return (
      <Box display="flex" justifyContent="center" alignItems="center" minHeight="80vh">
        <CircularProgress size={60} sx={{ color: MEHANDI_COLOR, thickness: 5 }} />
      </Box>
    );
  }
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 6 }}>
      ...
      <Box sx={{ mb: 6, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 3 }}>
        <Box>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1 }}>
            <Avatar sx={{ bgcolor: `${MEHANDI_COLOR}15`, color: MEHANDI_COLOR, width: 56, height: 56 }}>
          ...
          <Typography variant="h6" color="text.secondary" sx={{ fontWeight: 400, opacity: 0.8, ml: 9 }}>
            {isEdit 
              ...
              : 'Maximize your visibility across premium mehandi service areas.'
            }
          </Typography>
        </Box>
      ...
      <Box sx={{ 
        ...
        border: `1px dashed ${MEHANDI_COLOR}30`,
        display: 'flex', alignItems: 'center', gap: 2
      }}>
        <StarsIcon sx={{ color: MEHANDI_COLOR }} />
        <Typography variant="body1" fontWeight="600" color={MEHANDI_COLOR}>
          PRO FEATURE:
            ...
            <Paper sx={{ 
              ...
              <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  <Button size="small" onClick={handleSelectAll} sx={{ color: MEHANDI_COLOR, fontWeight: 'bold' }}>All</Button>
                ...
                {zones.map((zone) => (
                  <Grid item xs={12} sm={6} md={3} key={zone._id}>
                    <Box sx={{ 
                      ...
                      transition: 'all 0.2s',
                      border: selectedZones.includes(zone._id) ? `2.5px solid ${MEHANDI_COLOR}` : '2.5px solid transparent',
                      bgcolor: selectedZones.includes(zone._id) ? `${MEHANDI_COLOR}05` : '#fcfcfc',
                      '&:hover': { bgcolor: selectedZones.includes(zone._id) ? `${MEHANDI_COLOR}08` : '#f5f5f5' }
                    }}>
                      <FormControlLabel
                        control={
                          <Checkbox 
                            ...
                            sx={{ color: '#ddd', '&.Mui-checked': { color: MEHANDI_COLOR } }} 
                          />
                        }
                        label={
                          <Box>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            ...
            <Box>
              ...
              <Card sx={{ 
                ...
                background: GRADIENT,
                color: 'white',
                boxShadow: `0 20px 50px ${MEHANDI_COLOR}30`,
                overflow: 'visible',
                position: 'relative'
              }}>
                    ...
                    multiZones.map((zone) => (
                      <Chip 
                        ...
                        sx={{ 
                          ...
                          color: '#333',
                          '& .MuiChip-icon': { color: MEHANDI_COLOR }
                        }} 
                      />
                    ))
                  ) : (
      ...
      >
        <Alert
          ...
          icon={<CheckCircleIcon fontSize="inherit" sx={{ color: MEHANDI_COLOR }} />}
        >
