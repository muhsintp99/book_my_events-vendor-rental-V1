...
const THEME_COLOR = '#dd666eff'; // Coral theme for Emcee
...
function EmceeSchedules() {
    ...
    const fetchBookings = async () => {
        ...
        try {
            ...
            if (data.success) {
                // Filter specifically for Emcee related bookings
                const filtered = (data.data || []).filter(b => {
                    const type = String(b.moduleType || '').toLowerCase();
                    return type.includes('emcee') || type.includes('host');
                });
    ...
    const fetchModules = async () => {
        try {
            ...
            const filteredModules = modulesList.filter(module => {
                ...
                return moduleTitle.includes('emcee') || moduleTitle.includes('host');
            });
    ...
    const fetchEmceePackages = async () => {
        try {
            ...
            console.error('Fetch Emcee packages error:', err);
            setPackages([]);
        }
    };
                        ...
                        <Grid item xs={12} md={6} lg={4} key={b._id}>
                            <Paper sx={{ p: 3, borderRadius: '24px', position: 'relative', overflow: 'hidden', border: '1px solid #f0f0f0', boxShadow: '0 8px 20px rgba(0,0,0,0.02)' }}>
                                ...
                                <Stack spacing={1}>
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: '#666' }}><MicIcon sx={{ fontSize: 16 }} /><Typography variant="body2">Emcee Service</Typography></Box>
                                </Stack>
                            </Paper>
                        </Grid>
                    ))
