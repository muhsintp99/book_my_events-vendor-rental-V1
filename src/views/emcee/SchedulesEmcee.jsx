...
function EmceeSchedules() {
        ...
        try {
            ...
            if (data.success) {
                const filtered = (data.data || []).filter(b =>
                    String(b.moduleType || '').toLowerCase().includes('emcee') ||
                    String(b.moduleType || '').toLowerCase().includes('host')
                );
    ...
    const fetchModules = async () => {
        try {
            ...
            const filtered = modulesList.filter(m => {
                ...
                return t.includes('emcee') || t.includes('host');
            });
            setModules(filtered);
    ...
    const fetchPackages = async () => {
        try {
            const response = await fetch(`${API_BASE_URL}/api/emcee/vendor/${providerId}`);
            const data = await response.json();
                        ...
                        <Grid item xs={12} md={6} lg={4} key={b._id}>
                            <Paper sx={{ p: 3, borderRadius: '24px', position: 'relative', overflow: 'hidden', border: '1px solid #f0f0f0', boxShadow: '0 8px 20px rgba(0,0,0,0.02)' }}>
                                ...
                                <Stack spacing={1}>
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: '#666' }}><MicIcon sx={{ fontSize: 16 }} /><Typography variant="body2">Emcee & Host Service</Typography></Box>
                                </Stack>
                            </Paper>
                        </Grid>
                    ))
                    ...
                    <Box sx={{ mt: 2 }}>
                        {activeStep === 0 && (
                            <Fade in={activeStep === 0}><Stack spacing={3}>
                                ...
                                <TextField fullWidth label="Service Address" name="address" value={formData.address} onChange={handleInputChange} InputProps={{ startAdornment: <InputAdornment position="start"><HomeIcon sx={{ color: '#ef5350' }} /></InputAdornment> }} sx={{ '& .MuiOutlinedInput-root': { borderRadius: '16px' } }} />
                            </Stack></Fade>
                        )}
                        {activeStep === 1 && (
                            <Fade in={activeStep === 1}><Stack spacing={3}>
                                <FormControl fullWidth sx={{ '& .MuiOutlinedInput-root': { borderRadius: '16px' } }}>
                                    <InputLabel>Select Emcee Package</InputLabel>
                                    <Select label="Select Emcee Package" name="packageId" value={formData.packageId} onChange={handleInputChange}>
