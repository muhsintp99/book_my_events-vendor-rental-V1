...
function FloristSchedules() {
    ...
    const fetchBookings = async () => {
        ...
        try {
            ...
            if (data.success) {
                // Filter specifically for Mehandi related bookings
                const filtered = (data.data || []).filter(b =>
                    ['florist', 'florist & stage decoration'].includes(String(b.moduleType || '').toLowerCase())
                );
    ...
    const fetchModules = async () => {
        try {
            ...
            const filteredModules = modulesList.filter(module => {
                ...
                return moduleTitle === 'florist' || moduleTitle === 'florist & stage decoration';
            });
    ...
    const fetchFloristPackages = async () => {
        try {
            ...
            console.error('Fetch mehandi packages error:', err);
            setPackages([]);
        }
    };
                        ...
                        <Grid item xs={12} md={6} lg={4} key={b._id}>
                            <Paper sx={{ p: 3, borderRadius: '24px', position: 'relative', overflow: 'hidden', border: '1px solid #f0f0f0', boxShadow: '0 8px 20px rgba(0,0,0,0.02)' }}>
                                ...
                                <Stack direction="row" justifyContent="space-between" mb={2}>
                                    <Chip label={Array.isArray(b.timeSlot) ? b.timeSlot.join(', ') : (typeof b.timeSlot === 'object' ? b.timeSlot.label : b.timeSlot)} size="small" icon={<AccessTimeIcon sx={{ fontSize: 14 }} />} sx={{ bgcolor: 'rgba(239, 83, 80, 0.1)', color: '#ef5350', fontWeight: 700 }} />
                                ...
                                <Stack spacing={1}>
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: '#666' }}><SpaIcon sx={{ fontSize: 16 }} /><Typography variant="body2">Florist</Typography></Box>
                                </Stack>
                            </Paper>
                        </Grid>
                    ))
                        ...
                        {activeStep === 1 && (
                            <Fade in={activeStep === 1}>
                                <Stack spacing={3}>
                                    ...
                                    <TextField fullWidth label="Number of Guests" type="number" name="numberOfGuests" value={formData.numberOfGuests} onChange={handleInputChange} InputProps={{ startAdornment: <InputAdornment position="start"><PeopleIcon sx={{ color: '#ef5350' }} /></InputAdornment> }} sx={{ '& .MuiOutlinedInput-root': { borderRadius: '16px' } }} />
                                    <Box>
