...
function FloristSchedules() {
    ...
    try {
      ...
      if (data.success) {
        const filtered = (data.data || []).filter((b) => {
          ...
          return mType.includes('florist') || mType.includes('flower') || mType.includes('stage');
        });
  ...
  // Fix 1: fetchModules — always sync moduleId when modules load
  const fetchModules = async () => {
    try {
      ...
      // ✅ Priority filter: Florist first, then broader terms
      let filtered = modulesList.filter((m) => {
        const title = (m.title || m.name || '').toLowerCase();
        return title.includes('florist');
      });

      // Fallback 1: stage/flower
      if (filtered.length === 0) {
        filtered = modulesList.filter((m) => {
          const title = (m.title || m.name || '').toLowerCase();
          return title.includes('flower') || title.includes('stage');
        });
      }

      // Fallback 2: use ALL modules — moduleId will be overridden by packageId anyway
      if (filtered.length === 0) {
        filtered = modulesList;
        console.warn('⚠️ No florist module found. Will rely on backend auto-resolution.');
      }
 ...
 const fetchFloristPackages = async () => {
    try {
        const response = await fetch(`${API_BASE_URL}/api/florist/vendor/${providerId}`);
        const data = await response.json();
        const pkgs = data.success ? (data.data || []) : [];
        setPackages(pkgs);

        // ✅ Extract moduleId directly from the florist package
        if (pkgs.length > 0) {
            const firstPkg = pkgs[0];
            const floristModuleId = firstPkg.secondaryModule || firstPkg.module || firstPkg.moduleId;
            if (floristModuleId) {
                console.log('✅ Got florist moduleId from package:', floristModuleId);
                setFormData(prev => ({ ...prev, moduleId: floristModuleId }));
            }
        }
    } catch (err) {
        console.error('Fetch florist packages error:', err);
        setPackages([]);
    }
};
        ...
        >
          <Box
            sx={{
              ...
              justifyContent: 'center',
              borderRadius: '50%',
              bgcolor: isToday ? '#ef5350' : 'transparent',
              mb: { xs: '2px', md: '8px' }
            }}
          >
            {day}
          ...
          <Box
            sx={{
              ...
              borderRadius: '50%',
              bgcolor: isPast ? '#ccc' : getStatusColor(status)
            }}
          />
        </Box>
      );
...
const handleSubmit = async () => {
    ...
    try {
        // ✅ Get moduleId from selected package directly
        ...
        const resolvedModuleId = selectedPkg?.secondaryModule 
            ...
            || formData.moduleId  // fallback to whatever was set
            || null;
        ...
        // ✅ Only add moduleId if it came FROM the florist package itself
        if (resolvedModuleId) {
            bookingData.moduleId = resolvedModuleId;
        }
        // ✅ If no moduleId at all, don't send it — backend will auto-resolve
  ...
  // Fix 2: handleOpenDialog — always force moduleId from latest modules state
  const handleOpenDialog = () => {
    ...
    // ✅ Always re-fetch modules to ensure fresh moduleId
    fetchModules();
    ...
    <Box sx={{ width: '100%', p: { xs: '8px', md: '40px' }, bgcolor: '#fafafa', minHeight: '100vh' }}>
      ...
      >
        <IconButton
          ...
          sx={{ bgcolor: '#fff', border: '1px solid #f0f0f0', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}
        >
          <ChevronLeftIcon />
        </IconButton>
      ...
      <Box sx={{ bgcolor: '#fff', borderRadius: '12px', p: '24px', mb: '32px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
        <Stack direction="row" justifyContent="center" spacing={3} flexWrap="wrap">
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Typography color="#666">Not Available</Typography>
          </Box>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Box sx={{ width: 12, height: 12, borderRadius: '50%', bgcolor: '#66bb6a' }} />
            <Typography color="#666">Available</Typography>
          </Box>
        </Stack>
      </Box>
      ...
      >
        ...
        <Box sx={{ flex: 1 }}>
          ...
          <Typography color="#999" fontSize="14px">
            {selectedBookings.length === 0 ? 'Slots available for manual entry' : 'Reserved slots for this date'}
          </Typography>
        </Box>
        <Button
          variant="contained"
          ...
          disabled={(() => {
            const isPastMonth =
              currentDate.getFullYear() < todayYear || (currentDate.getFullYear() === todayYear && currentDate.getMonth() < todayMonth);
            const isPastDay =
              isPastMonth || (currentDate.getMonth() === todayMonth && currentDate.getFullYear() === todayYear && selectedDate < todayDate);
            return isPastDay;
          })()}
              ...
              >
                ...
                <Stack direction="row" justifyContent="space-between" mb={2}>
                  <Chip
                    label={
                      Array.isArray(b.timeSlot)
                        ...
                        : typeof b.timeSlot === 'object'
                          ? b.timeSlot.label || b.timeSlot.name || b.timeSlot.time
                          : b.timeSlot || 'Full Day'
                    }
                    size="small"
                  ...
                  <IconButton
                    ...
                    sx={{ color: '#ccc', '&:hover': { color: '#ef5350' } }}
                  >
                ...
                <Stack spacing={1}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: '#666' }}>
                    ...
                    <Typography variant="body2">{b.contactNumber}</Typography>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: '#666' }}>
                    <LocalFloristIcon sx={{ fontSize: 16 }} />
                    <Typography variant="body2">Florist Service</Typography>
                  </Box>
                </Stack>
              </Paper>
            </Grid>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: '28px', p: 1 } }}>
        <DialogTitle sx={{ position: 'relative' }}>
          ...
          <Typography variant="h4" fontWeight={800} sx={{ mt: 2, textAlign: 'center' }}>
          ...
          <Box sx={{ width: '80%', mx: 'auto', mt: 3, mb: 1 }}>
            <Stepper activeStep={activeStep} alternativeLabel>
              {steps.map((label) => (
                <Step
                  key={label}
                  sx={{ '& .MuiStepIcon-root.Mui-active': { color: '#ef5350' }, '& .MuiStepIcon-root.Mui-completed': { color: '#ef5350' } }}
                >
              ...
              <Fade in={activeStep === 1}>
                <Stack spacing={3}>
                  <FormControl fullWidth sx={{ '& .MuiOutlinedInput-root': { borderRadius: '16px' } }}>
                    <InputLabel>Select Florist Package</InputLabel>
                    <Select label="Select Florist Package" name="packageId" value={formData.packageId} onChange={handleInputChange}>
                      {packages.map((pkg) => (
                  ...
                  <Box>
                    ...
                    <TextField
                      fullWidth
                      disabled
                      value={new Date(currentDate.getFullYear(), currentDate.getMonth(), selectedDate).toLocaleDateString('en-US', {
                        dateStyle: 'full'
                      })}
...
export default FloristSchedules;
