        ...
        try {
            ...
            if (data.success) {
                const filtered = (data.data || []).filter(b =>
                    String(b.moduleType || '').toLowerCase().includes('florist') ||
                    String(b.moduleType || '').toLowerCase().includes('stage decoration')
                );
    ...
    const fetchModules = async () => {
        try {
            ...
            const filtered = modulesList.filter(m => {
                ...
                return t.includes('florist') || t.includes('stage decoration');
            });
            setModules(filtered);
                        ...
                        <Grid item xs={12} md={6} lg={4} key={b._id}>
                            <Paper sx={{ p: 3, borderRadius: '24px', position: 'relative', overflow: 'hidden', border: '1px solid #f0f0f0', boxShadow: '0 8px 20px rgba(0,0,0,0.02)' }}>
                                ...
                                <Stack spacing={1}>
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: '#666' }}><SpaIcon sx={{ fontSize: 16 }} /><Typography variant="body2">Florist & Stage Decoration</Typography></Box>
                                </Stack>
                            </Paper>
                        </Grid>
                    ))
