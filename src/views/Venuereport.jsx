    ...
    <Box sx={{ p: 2 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 4, backgroundColor: 'white', borderRadius: 2, p: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mr: 2 }}>
          <Typography variant="h6" sx={{ mr: 1 }}>📋 Venue Report</Typography>
        </Box>
      </Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', backgroundColor: 'white', p: 2, borderRadius: 2, mb: 4 }}>
        ...
        <Box sx={{ textAlign: 'center', p: 2, border: '1px solid #e0e0e0', borderRadius: 2, width: '18%', backgroundColor: '#e8f5e9' }}>
          ...
          <Typography variant="body2">In progress Venues <InfoIcon sx={{ fontSize: 16, color: '#4caf50' }} /></Typography>
          <Box sx={{ display: 'flex', justifyContent: 'center', mt: 1 }}>
        ...
        <Box sx={{ textAlign: 'center', p: 2, border: '1px solid #e0e0e0', borderRadius: 2, width: '18%', backgroundColor: '#e0f7fa' }}>
          ...
          <Typography variant="body2">Ongoing venues</Typography>
          <Box sx={{ display: 'flex', justifyContent: 'center', mt: 1 }}>
        ...
        <Box sx={{ textAlign: 'center', p: 2, border: '1px solid #e0e0e0', borderRadius: 2, width: '18%', backgroundColor: '#ffebee' }}>
          ...
          <Typography variant="body2">Completed venues</Typography>
          <Box sx={{ display: 'flex', justifyContent: 'center', mt: 1 }}>
        ...
        <Box sx={{ textAlign: 'center', p: 2, border: '1px solid #e0e0e0', borderRadius: 2, width: '18%', backgroundColor: '#fce4ec' }}>
          ...
          <Typography variant="body2">Canceled venues</Typography>
          <Box sx={{ display: 'flex', justifyContent: 'center', mt: 1 }}>
      ...
      <Box sx={{ backgroundColor: 'white', borderRadius: 2, p: 2 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
          <Typography variant="h6" sx={{ color: '#666' }}>
            Total Venues <span style={{ color: '#999', fontSize: '14px' }}>4</span>
          </Typography>
          ...
          <Table>
            <TableHead>
              <TableRow sx={{ bgcolor: '#f5f5f5' }}>
                ...
                <TableCell sx={{ fontWeight: 'bold', fontSize: '12px', padding: '8px', whiteSpace: 'nowrap' }}>Venue Id</TableCell>
                <TableCell sx={{ fontWeight: 'bold', fontSize: '12px', padding: '8px', whiteSpace: 'nowrap' }}>Customer Info</TableCell>
                <TableCell sx={{ fontWeight: 'bold', fontSize: '12px', padding: '8px', whiteSpace: 'nowrap' }}>Total Fare Of Venue</TableCell>
                <TableCell sx={{ fontWeight: 'bold', fontSize: '12px', padding: '8px', whiteSpace: 'nowrap' }}>Discount On Venue</TableCell>
                ...
                <TableCell sx={{ fontWeight: 'bold', fontSize: '12px', padding: '8px', whiteSpace: 'nowrap' }}>Total Venue Amount</TableCell>
                ...
                <TableCell sx={{ fontWeight: 'bold', fontSize: '12px', padding: '8px', whiteSpace: 'nowrap' }}>Venue Status</TableCell>
                <TableCell sx={{ fontWeight: 'bold', fontSize: '12px', padding: '8px', whiteSpace: 'nowrap' }}>Action</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {tripData.map((venue, index) => (
                <TableRow key={venue.id} sx={{ '&:hover': { bgcolor: '#f9f9f9' } }}>
                  ...
                  <TableCell sx={{ fontSize: '12px', padding: '8px' }}>
                    ...
                    <Chip
                      label={venue.totalAmountReceivedBy === 'Not Received Yet' ? 'Unpaid' : 'Paid'}
                      size="small"
                      sx={{
                        bgcolor: venue.totalAmountReceivedBy === 'Not Received Yet' ? '#f44336' : '#4caf50',
                        color: 'white',
                        fontSize: '10px',
                        height: '20px',
                        marginTop: '4px'
                  ...
                  <TableCell sx={{ fontSize: '12px', padding: '8px' }}>₹{venue.totalFare.toFixed(2)}</TableCell>
                  <TableCell sx={{ fontSize: '12px', padding: '8px' }}>₹{venue.discount.toFixed(2)}</TableCell>
                  <TableCell sx={{ fontSize: '12px', padding: '8px' }}>₹{venue.couponDiscount.toFixed(2)}</TableCell>
                  <TableCell sx={{ fontSize: '12px', padding: '8px' }}>₹{venue.couponDiscount.toFixed(2)}</TableCell>
                  <TableCell sx={{ fontSize: '12px', padding: '8px' }}>₹{venue.referralDiscount.toFixed(2)}</TableCell>
                  <TableCell sx={{ fontSize: '12px', padding: '8px' }}>₹{venue.totalDiscounted.toFixed(2)}</TableCell>
                  <TableCell sx={{ fontSize: '12px', padding: '8px' }}>₹{venue.tax.toFixed(2)}</TableCell>
                  <TableCell sx={{ fontSize: '12px', padding: '8px' }}>₹{venue.additionalCharge.toFixed(2)}</TableCell>
                  <TableCell sx={{ fontSize: '12px', padding: '8px' }}>
                    <Typography variant="body2" sx={{ fontWeight: 'bold' }}>
                      ₹{venue.totalVenueAmount.toFixed(2)}
                    </Typography>
                    <Chip
                      label={venue.totalAmountReceivedBy === 'Not Received Yet' ? 'Unpaid' : 'Paid'}
                      size="small"
                  ...
                  <TableCell sx={{ fontSize: '12px', padding: '8px' }}>{venue.totalAmountReceivedBy}</TableCell>
                  <TableCell sx={{ fontSize: '12px', padding: '8px' }}>{venue.paymentMethod}</TableCell>
                  <TableCell sx={{ fontSize: '12px', padding: '8px' }}>
                    <Chip
                      label={venue.venueStatus}
                      size="small"
                      sx={{
                        bgcolor: venue.venueStatus === 'Pending' ? '#bbdefb' : venue.venueStatus === 'Confirmed' ? '#b2dfdb' : '#c8e6c9',
                        color: venue.venueStatus === 'Pending' ? '#1976d2' : venue.venueStatus === 'Confirmed' ? '#00695c' : '#2e7d32',
                        fontSize: '10px',
                        height: '20px'
                      }}
                    />
                  ...
                  <TableCell sx={{ fontSize: '12px', padding: '8px' }}>
                    <Box sx={{ display: 'flex', gap: 1 }}>
                     <Button  onClick={() => navigate(`/report/tripdetail?id=${venue.id}`)}     variant="outlined" size="small" sx={{ color: '#1976d2', borderColor: '#1976d2' }}>
                                           {venue.actions}
                                         </Button>
