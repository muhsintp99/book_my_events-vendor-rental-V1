...
function LightSchedules() {
        ...
        try {
            ...
            if (data.success) {
                const filtered = (data.data || []).filter(b =>
                    String(b.moduleType || '').toLowerCase().includes('light') ||
                    String(b.moduleType || '').toLowerCase().includes('sound')
                );
    ...
    const fetchModules = async () => {
        try {
            ...
            const filtered = modulesList.filter(m => {
                ...
                return t.includes('light') || t.includes('sound');
            });
            setModules(filtered);
        } catch (err) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const bookingData = {
                ...
                lightAndSoundId: formData.packageId,
    ...
    const resetForm = () => {
        setFormData({
            ...
            additionalNotes: '', moduleId: modules[0]?._id || '',
            packageId: '', bookingDate: new Date().toISOString().split('T')[0],
                        ...
                        <Grid item xs={12} md={6} lg={4} key={b._id}>
                            <Paper sx={{ p: 3, borderRadius: '24px', position: 'relative', overflow: 'hidden', border: '1px solid #f0f0f0', boxShadow: '0 8px 20px rgba(0,0,0,0.02)' }}>
                                ...
                                <Stack spacing={1}>
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: '#666' }}><LightModeIcon sx={{ fontSize: 16 }} /><Typography variant="body2">Light & Sound</Typography></Box>
                                </Stack>
                            </Paper>
                        </Grid>
                    ))
                            ...
                            <Fade in={activeStep === 1}>
                                <Stack spacing={3}>
                                    <FormControl fullWidth sx={{ '& .MuiOutlinedInput-root': { borderRadius: '16px' } }}>
                                        <InputLabel>Select Light & Sound Package</InputLabel>
                                        <Select label="Select Light & Sound Package" name="packageId" value={formData.packageId} onChange={handleInputChange}>
