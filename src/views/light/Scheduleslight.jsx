...
function LightSchedules() {
    ...
    const fetchBookings = async () => {
        ...
        try {
            ...
            if (data.success) {
                // Filter specifically for Light & Sound related bookings
                const filtered = (data.data || []).filter(b => {
                    const type = String(b.moduleType || '').toLowerCase();
                    return type.includes('light') || type.includes('sound');
                });
    ...
    const fetchModules = async () => {
        try {
            ...
            const filteredModules = modulesList.filter(module => {
                ...
                return moduleTitle.includes('light') || moduleTitle.includes('sound');
            });
    ...
    const fetchLightPackages = async () => {
        try {
            const response = await fetch(`${API_BASE_URL}/api/light/vendor/${providerId}`);
            const data = await response.json();
            if (data.success) {
            ...
            console.error('Fetch light packages error:', err);
            setPackages([]);
        }
    };
    ...
    return (
        <Box sx={{ width: '100%', p: { xs: '8px', md: '40px' }, bgcolor: '#fafafa', minHeight: '100vh' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 2, md: 4 }, bgcolor: '#fff', borderRadius: '24px', p: { xs: '20px', md: '32px' }, mb: '32px', border: '1px solid #f0f0f0', boxShadow: '0 4px 20px rgba(0,0,0,0.03)' }}>
                ...
                <Button variant="contained" onClick={handleOpenDialog} sx={{ bgcolor: THEME_COLOR, borderRadius: '12px', px: 4, height: 48, fontWeight: 700, '&:hover': { bgcolor: '#5e35b1' } }}>Add Booking</Button>
            </Box>
                        ...
                        <Grid item xs={12} md={6} lg={4} key={b._id}>
                            <Paper sx={{ p: 3, borderRadius: '24px', position: 'relative', overflow: 'hidden', border: '1px solid #f0f0f0', boxShadow: '0 8px 20px rgba(0,0,0,0.02)' }}>
                                ...
                                <Stack spacing={1}>
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: '#666' }}><HighlightIcon sx={{ fontSize: 16 }} /><Typography variant="body2">Light & Sound Service</Typography></Box>
                                </Stack>
                            </Paper>
                        </Grid>
                    ))
                ...
                <DialogContent sx={{ px: 4, pb: 4, pt: 2, minHeight: 400 }}>
                            ...
                            <Fade in={activeStep === 1}>
                                <Stack spacing={3}>
                                    <FormControl fullWidth sx={{ '& .MuiOutlinedInput-root': { borderRadius: '16px' } }}>
                                        <InputLabel>Select Light Package</InputLabel>
                                        <Select label="Select Light Package" name="packageId" value={formData.packageId} onChange={handleInputChange}>
                    ...
                    <Stack direction="row" spacing={2} sx={{ mt: 6 }}>
                        ...
                        {activeStep < steps.length - 1 ? (
                            <Button fullWidth variant="contained" onClick={handleNext} sx={{ borderRadius: '16px', fontWeight: 700, height: 55, bgcolor: THEME_COLOR, '&:hover': { bgcolor: '#5e35b1' } }}>Next Step</Button>
                        ) : (
                            <Button fullWidth variant="contained" onClick={handleSubmit} disabled={submitLoading} sx={{ borderRadius: '16px', fontWeight: 700, height: 55, bgcolor: THEME_COLOR, boxShadow: `0 8px 16px ${alpha(THEME_COLOR, 0.3)}`, '&:hover': { bgcolor: '#5e35b1' } }}>{submitLoading ? <CircularProgress size={24} color="inherit" /> : 'Confirm Booking'}</Button>
                        )}
                    </Stack>
                </DialogContent>
            </Dialog>
