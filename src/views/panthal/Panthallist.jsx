...
const API_BASE_URL = 'https://api.bookmyevent.ae/api/panthal-decoration'; // Backend panthal endpoint

export default function PanthalList() {
    const navigate = useNavigate();
    const [panthalPackages, setPanthalPackages] = useState([]);
    ...
    const fetchPackages = useCallback(async () => {
        ...
        try {
            ...
            if (result.success) setPanthalPackages(result.data || []);
        } catch (err) {
    ...
    const handleToggleStatus = async (id, current) => {
        try {
            ...
            if (data.success) {
                setPanthalPackages((prev) => prev.map((m) => (m._id === id ? { ...m, isActive: !current } : m)));
    ...
    const handleDelete = async () => {
        try {
            ...
            const data = await res.json();
            if (data.success) {
                setPanthalPackages((prev) => prev.filter((m) => m._id !== packageToDelete));
    ...
    const filteredPackages = useMemo(() => {
        return panthalPackages.filter((m) => {
    ...
    }, [panthalPackages, searchQuery]);

    const THEME_COLOR = '#C62828'; // Deep red theme color for Panthal

    return (
        <Box sx={{ bgcolor: '#f5f7fa', minHeight: '100vh', pb: 4 }}>
            <Box
                sx={{
                    background: `linear-gradient(135deg, ${THEME_COLOR} 0%, #8E0000 100%)`,
                    color: 'white',
                    py: 4,
                    px: 3,
                    ...
                    <Stack direction="row" justifyContent="space-between" alignItems="center">
                        <Box>
                            ...
                            <Typography variant="body2" sx={{ opacity: 0.9 }}>
                                {panthalPackages.length} total packages • {panthalPackages.filter((m) => m.isActive).length} active
                            </Typography>
                        </Box>
                        <Button
                            variant="contained"
            ...
            <Box sx={{ width: '100%', px: 2, mt: 4 }}>
                ...
                {loading ? (
                    ...
                    <Card sx={{ textAlign: 'center', py: 8 }}>
                        <Celebration sx={{ fontSize: 80, color: '#ddd', mb: 2 }} />
                                ...
                                <Card sx={{ height: '100%', borderRadius: 3, transition: '0.3s', '&:hover': { transform: 'translateY(-8px)' } }}>
                                    <Box sx={{ position: 'relative', pt: '75%', bgcolor: '#f5f5f5' }}>
                                        {pkg.image ? (
                                            ...
                                            <Box sx={{ position: 'absolute', top: 0, width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                <Celebration sx={{ fontSize: 60, color: '#ccc' }} />
                                            </Box>
                                        )}
                                    </Box>
                                    <CardContent sx={{ p: 2.5 }}>
                                            ...
                                            {pkg.services && pkg.services.length > 0 ? (
                                                pkg.services.map((svc) => (
                                                    <Chip
                                                        ...
                                                        size="small"
                                                        sx={{ fontSize: '0.65rem', height: '28px', bgcolor: 'rgba(198, 40, 40, 0.1)', color: THEME_COLOR }}
                                                    />
                                                ))
                                            ) : (
                                        ...
                                        <Stack direction="row" spacing={1}>
                                            ...
                                            <IconButton color="primary" onClick={() => navigate(`/panthal/edit/${pkg._id}`)}><Edit /></IconButton>
            ...
            <Dialog open={openView} onClose={() => setOpenView(false)} maxWidth="md" fullWidth>
                ...
                <DialogActions sx={{ p: 2 }}>
                    ...
                    <Button variant="contained" sx={{ bgcolor: THEME_COLOR }} onClick={() => navigate(`/panthal/edit/${selectedPackage?._id}`)}>Edit Package</Button>
                </DialogActions>
            </Dialog>
