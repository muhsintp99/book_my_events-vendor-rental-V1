    ...
    return (
        <Box p={2}>
            <Typography variant="h4" gutterBottom sx={{ color: THEME, fontWeight: 700 }}>
                Panthal & Decoration Enquiries
            </Typography>
            {error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}
            <Paper sx={{ mt: 2 }}>
                <Box p={2}>
                    <TextField label="Search enquiries" size="small" value={search}
                                ...
                                <TableRow key={e._id} hover>
                                    ...
                                    <TableCell align="center">
                                        <Stack direction="row" spacing={1} justifyContent="center">
                                            <Tooltip title="Chat">
                                                <IconButton size="small" sx={{ color: THEME }}
                                                    onClick={() => navigate('/panthal/enquirychat', { state: { enquiry: e } })}>
                                                    <ChatIcon fontSize="small" />
                                                </IconButton>
                                            </Tooltip>
                                            <Tooltip title="View">
                                                <IconButton size="small" color="info" onClick={() => handleViewEnquiry(e)}>
            ...
            <Dialog open={openModal} onClose={handleCloseModal} maxWidth="md" fullWidth>
                ...
                <DialogActions>
                    ...
                    <Button variant="contained" startIcon={<ChatIcon />}
                        sx={{ bgcolor: THEME, '&:hover': { bgcolor: '#8E0000' } }}
                        onClick={() => {
                            setChatEnquiry(selectedEnquiry); setOpenChat(true);
                            navigate('/panthal/enquirychat', { state: { enquiry: selectedEnquiry } });
                        }}>
                        Open Chat
                    </Button>
                </DialogActions>
            ...
            <EnquiryChatDialog open={openChat} onClose={() => { setOpenChat(false); setChatEnquiry(null); }} enquiry={chatEnquiry} />
        </Box>
    );
};

export default PanthalEnquiries;
