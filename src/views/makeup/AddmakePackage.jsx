  ...
  return (
    <Box sx={{ position: 'relative', border: '1px solid #e5e5e5', borderRadius: 2, p: 3, mb: 3, bgcolor: 'white' }}>
      ...
      <TextField
        ...
        label={
          <span>
            Items (comma separated) <span style={{ color: 'red' }}>*</span>
          </span>
        }
...
const AddmakePackage = () => {
  ...
  const [serviceSections, setServiceSections] = useState([{ id: Date.now(), title: '', itemsText: '' }]);
  ...
  const resetForm = () => {
    ...
    setIsActive(true);
    setServiceSections([{ id: Date.now(), title: '', itemsText: '' }]);
    setBasicServices({
      hairStyling: false,
      sareeDraping: false,
      eyelashExtension: false,
  ...
  const handleAddSection = () => {
    setServiceSections((prev) => [...prev, { id: Date.now(), title: '', itemsText: '' }]);
  };

  const handleSectionChange = (id, field, value) => {
    setServiceSections((prev) => prev.map((sec) => (sec.id === id ? { ...sec, [field]: value } : sec)));
  };
