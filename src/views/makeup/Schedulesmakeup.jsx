...
function MakeupSchedules() {
    ...
    try {
      ...
      if (data.success) {
        const filtered = (data.data || []).filter(b =>
          String(b.moduleType || '').toLowerCase().includes('makeup')
        );
  ...
  const fetchPackages = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/makeup-packages/provider/${providerId}`);
      const data = await response.json();
            ...
            <Grid item xs={12} md={6} lg={4} key={b._id}>
              <Paper sx={{ p: 3, borderRadius: '24px', position: 'relative', overflow: 'hidden', border: '1px solid #f0f0f0', boxShadow: '0 8px 20px rgba(0,0,0,0.02)' }}>
                ...
                <Stack spacing={1}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: '#666' }}><BrushIcon sx={{ fontSize: 16 }} /><Typography variant="body2">Makeup Artist</Typography></Box>
                </Stack>
              </Paper>
            </Grid>
          ))
            ...
            {activeStep === 1 && (
              <Fade in={activeStep === 1}><Stack spacing={3}>
                <FormControl fullWidth sx={{ '& .MuiOutlinedInput-root': { borderRadius: '16px' } }}>
                  <InputLabel>Select Makeup Package</InputLabel>
                  <Select label="Select Makeup Package" name="packageId" value={formData.packageId} onChange={handleInputChange}>
...
export default MakeupSchedules;