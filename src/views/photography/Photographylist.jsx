      ...
      <Paper sx={{ p: 2, mt: 3 }}>
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems="center">
          ...
          <Box sx={{ ml: 'auto', gap: 1, display: 'flex' }}>
            <Button variant="outlined" startIcon={<Refresh />} onClick={fetchPhotography}>
              Refresh
            </Button>
            ...
            <TableBody>
              ...
              {filteredPackages.map((p, i) => (
                <TableRow key={p._id} hover>
                  ...
                  <TableCell>
                    <ImageAvatar src={getImageUrl(p.gallery?.[0])} alt={p.packageTitle} title={p.packageTitle} />
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <Stack direction="row" spacing={0.5} flexWrap="wrap">
                      {p.categories?.length > 0 ? (
                        p.categories.map((c) => <Chip key={c._id} label={c.title} size="small" color="primary" variant="outlined" />)
                      ) : (
      ...
      <Dialog open={openView} onClose={() => setOpenView(false)} maxWidth="md" fullWidth scroll="paper">
                      ...
                      <Grid container spacing={1} sx={{ mt: 2 }}>
                        {selectedPackage.gallery.map((img, idx) => (
                          <Grid item xs={3} sm={2} key={idx}>
                            ...
                            >
                              <img src={getImageUrl(img)} alt={`thumb-${idx}`} style={{ width: '100%', height: 80, objectFit: 'cover' }} />
                            </Box>
                          </Grid>
                        ))}
                      </Grid>
                ...
                <Grid container spacing={3}>
                  <Grid item xs={12} sm={6}>
                    <DetailItem
                      ...
                      value={selectedPackage.advanceBookingAmount || 'Not specified'}
                    />
                  </Grid>
                  <Grid item xs={12} sm={6}>
                    <DetailItem
                      ...
                      value={selectedPackage.travelToVenue ? 'Yes' : 'No'}
                      chip
                      success={selectedPackage.travelToVenue}
                    />
                  </Grid>
                  <Grid item xs={12} sm={6}>
                    <DetailItem
                      ...
                      chip
                      success={selectedPackage.isActive}
                      error={!selectedPackage.isActive}
                    />
                  </Grid>
        ...
        <DialogActions sx={{ p: 3, bgcolor: '#f9f9f9' }}>
          <Button
            ...
            onClick={() => {
              setOpenView(false);
              handleEditOpen(selectedPackage);
            }}
          >
            Edit Package
          </Button>
          ...
          <Grid container spacing={3} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12}>
              <FormControlLabel
                control={
                  <Checkbox
                    checked={editForm.travelToVenue}
                    onChange={(e) => setEditForm({ ...editForm, travelToVenue: e.target.checked })}
                  />
                }
                label="Travel to Venue"
              />
              <FormControlLabel
                control={
                  <Checkbox checked={editForm.isActive} onChange={(e) => setEditForm({ ...editForm, isActive: e.target.checked })} />
                }
