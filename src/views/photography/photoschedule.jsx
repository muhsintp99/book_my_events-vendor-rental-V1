...
function PhotographySchedules() {
  ...
  const getProviderId = () => {
    ...
    if (!id) {
      ...
      if (userStr) {
        try { const user = JSON.parse(userStr); id = user._id || user.id || user.userId; } catch (e) { }
      }
    }
    return id;
  };
  ...
  useEffect(() => {
    if (providerId) { fetchBookings(); fetchModules(); fetchPackages(); }
  ...
  const fetchModules = async () => {
    try {
      ...
      const filtered = modulesList.filter(m => (m.title || m.name || '').toLowerCase() === 'photography');
      setModules(filtered);
  ...
  const fetchPackages = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/photography-packages/provider/${providerId}`);
      const data = await response.json();
  ...
  const renderCalendarDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const isPast = new Date(currentDate.getFullYear(), currentDate.getMonth(), day) < new Date(todayYear, todayMonth, todayDate);
  ...
  const resetForm = () => {
    setFormData({ fullName: '', contactNumber: '', emailAddress: '', address: '', additionalNotes: '', moduleId: modules[0]?._id || '', packageId: '', bookingDate: new Date().toISOString().split('T')[0], timeSlot: ['Full Day'], paymentType: 'Cash', bookingType: 'Direct' });
    setActiveStep(0);
  };
            ...
            <Grid item xs={12} md={6} lg={4} key={b._id}>
              <Paper sx={{ p: 3, borderRadius: '24px', position: 'relative', overflow: 'hidden', border: '1px solid #f0f0f0', boxShadow: '0 8px 20px rgba(0,0,0,0.02)' }}>
                ...
                <Stack spacing={1}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: '#666' }}><CameraAltIcon sx={{ fontSize: 16 }} /><Typography variant="body2">{renderSafe(b.photographyId?.packageName || 'Photography')}</Typography></Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: '#666' }}><AccessTimeIcon sx={{ fontSize: 16 }} /><Typography variant="body2">{Array.isArray(b.timeSlot) ? b.timeSlot.join(', ') : b.timeSlot || 'Full Day'}</Typography></Box>
                </Stack>
              </Paper>
            </Grid>
          ))
            ...
            {activeStep === 1 && (
              <Fade in={activeStep === 1}><Stack spacing={3}>
                <FormControl fullWidth sx={{ '& .MuiOutlinedInput-root': { borderRadius: '16px' } }}>
                  <InputLabel>Select Photography Package</InputLabel>
                  <Select label="Select Photography Package" name="packageId" value={formData.packageId} onChange={handleInputChange}>
                ...
                <FormControl fullWidth sx={{ '& .MuiOutlinedInput-root': { borderRadius: '16px' } }}>
                  ...
                  <Select
                    ...
                    onChange={(e) => setFormData(prev => ({ ...prev, timeSlot: typeof e.target.value === 'string' ? e.target.value.split(',') : e.target.value }))}
                  ...
                  >
                    {['Full Day', 'Morning', 'Afternoon', 'Evening', 'Night'].map((slot) => (
...
export default PhotographySchedules;