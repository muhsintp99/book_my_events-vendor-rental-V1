...
const getImageUrl = (path) => {
  ...
  let normalized = String(path || '').toLowerCase().replace(/^uploads/i, '/uploads');
...
const AddNewMenu = () => {
  ...
  const populateForm = (pkg) => {
    ...
    setMenuSections((pkg.includes || []).map((inc, i) => ({ id: Date.now() + i, heading: inc.title || '', includes: (inc.items || []).join(', ') })));
  ...
  const handleCloseToast = (e, reason) => { if (reason === 'clickaway') return; setOpenToast(false); };
  ...
  const handleSubmit = async (e) => {
    ...
    const transformedIncludes = menuSections
      ...
      .map((s) => ({ title: s.heading.trim() || 'Section', items: s.includes.split(',').map(i => i.trim()).filter(Boolean) }));
      ...
      <Box sx={{ p: 3, backgroundColor: theme.palette.grey[100], minHeight: '100vh', width: '100%' }}>
        <Box sx={{ width: '100%', margin: 'auto', backgroundColor: 'white', borderRadius: theme.shape.borderRadius, boxShadow: theme.shadows[1], p: 3, overflowX: 'hidden' }}>
          ...
          <Box sx={{ position: 'relative', mb: 3 }}>
            <img src={getImageUrl(currentPackage.thumbnail)} alt={currentPackage.title} style={{ width: '100%', height: 200, objectFit: 'cover', borderRadius: 8 }} onError={(e) => { e.target.onerror = null; e.target.src = '/placeholder.jpg'; }} />
          ...
          <Typography variant="body1" sx={{ mb: 3, textAlign: 'justify' }}>{currentPackage.description || 'No description available.'}</Typography>
          ...
          {(currentPackage.includes || []).map((inc, idx) => (
            <Box key={idx} sx={{ mb: 2 }}>
              <Typography variant="subtitle1" sx={{ mb: 1, fontWeight: 'medium' }}>{inc.title || 'Untitled Section'}</Typography>
              <Stack component="ul" spacing={0.5} sx={{ pl: 2, mb: 0 }}>
                {inc.items?.map((it, i) => <Typography key={i} variant="body2" component="li">{it}</Typography> ) || <Typography variant="body2" color="text.secondary">No items</Typography>}
              </Stack>
            </Box>
          ))}
          ...
          <Box sx={{ textAlign: 'center', mb: 3 }}>
            ...
            <Typography variant="h4" color="primary">₹{currentPackage.price || 0} Per Head</Typography>
          </Box>
            ...
            <Stack direction="row" spacing={2} sx={{ flexWrap: 'wrap', justifyContent: 'center' }}>
              {(currentPackage.images || []).slice(0, 6).map((img, i) => (
                <Box key={i} sx={{ position: 'relative' }}>
                  <img src={getImageUrl(img)} alt={`Gallery ${i+1}`} style={{ width: 150, height: 150, objectFit: 'cover', borderRadius: 8 }} onError={(e) => { e.target.onerror = null; e.target.src = '/placeholder.jpg'; }} />
                </Box>
              ))}
            </Stack>
          </Box>
          ...
          <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center' }}>
            <Button variant="outlined" startIcon={<EditIcon />} onClick={handleEdit} sx={{ color: '#E15B65', borderColor: '#E15B65' }}>Edit Menu</Button>
          </Box>

          <Snackbar open={openToast} autoHideDuration={6000} onClose={handleCloseToast} anchorOrigin={{ vertical: 'top', horizontal: 'center' }}>
            <Alert onClose={handleCloseToast} severity={toastSeverity} sx={{ width: '100%' }}>{toastMessage}</Alert>
          </Snackbar>
        </Box>
      </Box>
    );
      ...
      <Box sx={{ width: '100%', margin: 'auto', backgroundColor: 'white', borderRadius: theme.shape.borderRadius, boxShadow: theme.shadows[1], p: 3, overflowX: 'hidden' }}>
        ...
        <Box component="form" onSubmit={handleSubmit}>
          ...
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField {...params} label="Category" placeholder={loadingCategories ? 'Loading categories...' : 'Select a category'} sx={{ mb: 2 }} variant="outlined" disabled={loadingCategories} />
            )}
          />
          ...
          <Box sx={{ mb: 3 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
              ...
              <Button variant="contained" size="small" onClick={addMenuSection} startIcon={<AddIcon />} sx={{ backgroundColor: '#E15B65', color: 'white' }}>Add</Button>
            </Box>
            ...
            {menuSections.map((section) => (
              <Box key={section.id} sx={{ border: `1px solid ${theme.palette.grey[300]}`, borderRadius: 1, p: 2, mb: 2 }}>
                ...
                <Box sx={{ display: 'flex', gap: 1, alignItems: 'flex-end' }}>
                  ...
                  <IconButton onClick={() => deleteMenuSection(section.id)} color="error" sx={{ alignSelf: 'flex-end' }}><DeleteIcon /></IconButton>
                </Box>
              </Box>
            ))}
          </Box>
                ...
                {thumbnailFile ? (
                  <Box sx={{ textAlign: 'center' }}>
                    <ImagePreviewContainer>
                      ...
                      <IconButton onClick={clearThumbnail} size="small" sx={{ position: 'absolute', top: -8, right: -8, backgroundColor: 'white', color: 'error', boxShadow: theme.shadows[2] }}><CloseIcon fontSize="small" /></IconButton>
                    </ImagePreviewContainer>
                    ...
                    <Stack direction="row" flexWrap="wrap" spacing={1} sx={{ justifyContent: 'center', mb: 1 }}>
                      {galleryFiles.map((file, i) => (
                        <ImagePreviewContainer key={i}>
                          <img src={URL.createObjectURL(file)} alt={`Gallery preview ${i + 1}`} style={{ width: 100, height: 100, objectFit: 'cover', borderRadius: 4 }} />
                          <IconButton onClick={() => deleteGalleryFile(i)} size="small" sx={{ position: 'absolute', top: -8, right: -8, backgroundColor: 'white', color: 'error', boxShadow: theme.shadows[2] }}><CloseIcon fontSize="small" /></IconButton>
                        </ImagePreviewContainer>
                      ))}
                    </Stack>
                ...
                <Box sx={{ mt: 2 }}>
                  ...
                  <Stack direction="row" flexWrap="wrap" spacing={1}>
                    {currentPackage.images.map((img, i) => (
                      <Box key={i} sx={{ position: 'relative' }}>
                        <img src={getImageUrl(img)} alt={`Existing gallery ${i + 1}`} style={{ width: 100, height: 100, objectFit: 'cover', borderRadius: 4 }} onError={(e) => { e.target.onerror = null; e.target.src = '/placeholder.jpg'; }} />
                      </Box>
                    ))}
                  </Stack>
                </Box>
          ...
          <Button type="submit" variant="contained" fullWidth sx={{ backgroundColor: '#E15B65', color: 'white', py: 1.5 }}>{currentPackage ? 'Update Menu' : 'Create Menu'}</Button>
        </Box>
