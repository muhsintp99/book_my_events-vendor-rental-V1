...
function BookingCalendar() {
  ...
  useEffect(() => {
    if (providerId) {
      fetchBookings();
      fetchModules();
      fetchCateringPackages(); // ✅ ADD THIS LINE
    }
  ...
  const fetchCateringPackages = async () => {
    try {
      ...
      console.error('Fetch catering packages error:', err);
      setPackages([]);
    }
  };
              ...
              >
                    ...
                    <Typography sx={{
                      ...
                      {Array.isArray(booking.timeSlot)
                        ? booking.timeSlot.map((t, i) => (
                          <span key={i}>
                            {typeof t === 'string' ? t : `${t.label} (${t.time})`}{i < booking.timeSlot.length - 1 ? ', ' : ''}
                          </span>
                        ))
                        : booking.timeSlot?.label
                          ? `${booking.timeSlot.label} (${booking.timeSlot.time})`
                          : (typeof booking.timeSlot === 'string' ? booking.timeSlot : 'All Day')}
                    </Typography>
                ...
                <Box sx={{
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    ...
                    <Box>
                      <Typography sx={{ fontSize: '11px', color: '#999', textTransform: 'uppercase', fontWeight: '600' }}>Service</Typography>
                      <Typography sx={{ fontSize: '14px', fontWeight: '500' }}>{booking.cateringId?.name || (booking.packageId?.packageName) || 'Catering'}</Typography>
                    </Box>
                  </Box>
                </Box>
              </Box>
            ...
            <Box mb={4}>
                ...
                >
                  ...
                  {packages.map(pkg => (
                    <MenuItem key={pkg._id} value={pkg._id}>
                      {pkg.name || pkg.packageName || pkg.packageTitle || pkg.title}
                    </MenuItem>
                  ))}
                </Select>
              </FormControl>
              {selectedPackage && (
                <Alert severity="info" sx={{ mb: 2 }}>
                  Selected Package: {selectedPackage.name || selectedPackage.packageName || selectedPackage.packageTitle || selectedPackage.title}
                </Alert>
              )}
