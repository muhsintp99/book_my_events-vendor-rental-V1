...
function CateringSchedules() {
    ...
    try {
      ...
      if (data.success) {
        const filtered = (data.data || []).filter(b =>
          String(b.moduleType || '').toLowerCase().includes('catering')
        );
  ...
  const fetchModules = async () => {
    try {
      ...
      const filtered = modulesList.filter(m => (m.title || m.name || '').toLowerCase() === 'catering');
      setModules(filtered);
...
export default CateringSchedules;
