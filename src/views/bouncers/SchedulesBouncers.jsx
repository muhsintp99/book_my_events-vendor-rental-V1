...
const THEME_COLOR = '#4caf50'; // Using green for bouncers/security or keep same consistent theme
...
function BouncersSchedules() {
    ...
    const fetchBookings = async () => {
        ...
        try {
            ...
            if (data.success) {
                // Filter specifically for Bouncers related bookings
                const filtered = (data.data || []).filter(b => {
                    const type = String(b.moduleType || '').toLowerCase();
                    return type.includes('bouncer') || type.includes('security');
                });
    ...
    const fetchModules = async () => {
        try {
            ...
            const filteredModules = modulesList.filter(module => {
                ...
                return moduleTitle.includes('bouncer') || moduleTitle.includes('security');
            });
    ...
    const fetchBouncersPackages = async () => {
        try {
            ...
            console.error('Fetch bouncers packages error:', err);
            setPackages([]);
        }
    };
                        ...
                        <Grid item xs={12} md={6} lg={4} key={b._id}>
                            <Paper sx={{ p: 3, borderRadius: '24px', position: 'relative', overflow: 'hidden', border: '1px solid #f0f0f0', boxShadow: '0 8px 20px rgba(0,0,0,0.02)' }}>
                                ...
                                <Stack spacing={1}>
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: '#666' }}><SecurityIcon sx={{ fontSize: 16 }} /><Typography variant="body2">Bouncer Service</Typography></Box>
                                </Stack>
                            </Paper>
                        </Grid>
                    ))
                            ...
                            <Fade in={activeStep === 1}>
                                <Stack spacing={3}>
                                    <FormControl fullWidth sx={{ '& .MuiOutlinedInput-root': { borderRadius: '16px' } }}>
                                        <InputLabel>Select Bouncer Package</InputLabel>
                                        <Select label="Select Bouncer Package" name="packageId" value={formData.packageId} onChange={handleInputChange}>
