        ...
        try {
            ...
            if (data.success) {
                const filtered = (data.data || []).filter(b =>
                    String(b.moduleType || '').toLowerCase().includes('bouncer') ||
                    String(b.moduleType || '').toLowerCase().includes('security')
                );
    ...
    const fetchModules = async () => {
        try {
            ...
            const filtered = modulesList.filter(m => {
                ...
                return t.includes('bouncer') || t.includes('security');
            });
            setModules(filtered);
                        ...
                        <Grid item xs={12} md={6} lg={4} key={b._id}>
                            <Paper sx={{ p: 3, borderRadius: '24px', position: 'relative', overflow: 'hidden', border: '1px solid #f0f0f0', boxShadow: '0 8px 20px rgba(0,0,0,0.02)' }}>
                                ...
                                <Stack spacing={1}>
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: '#666' }}><SecurityIcon sx={{ fontSize: 16 }} /><Typography variant="body2">Bouncers & Security</Typography></Box>
                                </Stack>
                            </Paper>
                        </Grid>
                    ))
                        ...
                        {activeStep === 1 && (
                            <Fade in={activeStep === 1}><Stack spacing={3}>
                                <FormControl fullWidth sx={{ '& .MuiOutlinedInput-root': { borderRadius: '16px' } }}>
                                    <InputLabel>Select Bouncers Package</InputLabel>
                                    <Select label="Select Bouncers Package" name="packageId" value={formData.packageId} onChange={handleInputChange}>
...
export default BouncersSchedules;
