...
//           }),
//           axios.get(`${API_BASE_URL}/categories/module/${moduleId}`, {
//             headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
//           }),
//         ]);
//         setBrands(brandsResponse.data.data || []);
//         setCategories(categoriesResponse.data.data || []);
//         console.log('Brands fetched:', brandsResponse.data.data);
//         console.log('Categories fetched:', categoriesResponse.data.data);
//       } catch (error) {
...
//     // Validation
//     if (!name || !brand || !model || !category || !type || !engineCapacity || !enginePower || !seatingCapacity || !fuelType || !transmissionType || !vinNumber || !licensePlateNumber || !tripType) {
...
//     if ((tripType === 'hourly' && !hourlyWisePrice) || (tripType === 'perDay' && !perDayPrice) || (tripType === 'distanceWise' && !distanceWisePrice)) {
...
//     if (!vehicleId) return;
//     try {
//       await axios.patch(`${API_BASE_URL}/vehicles/${vehicleId}/block`, {}, {
//         headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
//       });
...
//     if (!vehicleId) return;
//     try {
//       await axios.patch(`${API_BASE_URL}/vehicles/${vehicleId}/reactivate`, {}, {
//         headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
//       });
...
//   // Fallback options for brands and categories
//   const fallbackBrands = [
//     { _id: '6700b8f1c8b9a4c2d3e4f5a1', title: 'Nissan' },
//     { _id: '6700b8f1c8b9a4c2d3e4f5a2', title: 'Toyota' },
//     { _id: '6700b8f1c8b9a4c2d3e4f5a3', title: 'Honda' },
//   ];
//   const fallbackCategories = [
//     { _id: '6700b8f1c8b9a4c2d3e4f5b1', title: 'SUV' },
//     { _id: '6700b8f1c8b9a4c2d3e4f5b2', title: 'Sedan' },
//     { _id: '6700b8f1c8b9a4c2d3e4f5b3', title: 'Minibus' },
//   ];
...
//                         {(brands.length > 0 ? brands : fallbackBrands).map((b) => (
...
const Createnew = ({ vehicleId }) => {
  ...
  useEffect(() => {
    ...
    if (vehicleData) {
      console.log('Vehicle data from navigation state:', vehicleData); // Debug log
      setName(vehicleData.name || '');
      setDescription(vehicleData.description || '');
      setBrand(vehicleData.brand?._id || '');
      setCategory(vehicleData.category?._id || '');
      setModel(vehicleData.model || '');
      setType(vehicleData.type || '');
      setEngineCapacity(vehicleData.engineCapacity?.toString() || '');
      setEnginePower(vehicleData.enginePower?.toString() || '');
      setSeatingCapacity(vehicleData.seatingCapacity?.toString() || '');
      setAirCondition(vehicleData.airCondition ? 'yes' : 'no');
      setFuelType(vehicleData.fuelType || '');
      setTransmissionType(vehicleData.transmissionType || '');
      setVinNumber(vehicleData.vinNumber || '');
      setLicensePlateNumber(vehicleData.licensePlateNumber || '');
      setSearchTags(vehicleData.searchTags || []);
      if (vehicleData.pricing) {
        setTripType(vehicleData.pricing.type || 'hourly');
        setHourlyWisePrice(vehicleData.pricing.hourly?.toString() || '');
        setPerDayPrice(vehicleData.pricing.perDay?.toString() || '');
        setDistanceWisePrice(vehicleData.pricing.distance?.toString() || '');
      }
      setDiscount(vehicleData.discount?.toString() || '');
    } else if (vehicleId) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      handleReset();
      if (vehicleId) {
        navigate('/vehicle-setup/lists'); // Navigate back to list page after update
      }
    } catch (error) {
      let errorMsg = error.response?.data?.message || 'Failed to save vehicle';
      if (error.response?.data?.errors) {
