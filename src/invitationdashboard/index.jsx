...
export default function Dashboard() {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      // 2. Strict Invitation Filter (pure matching to prevent bleed)
      const invitationBookings = allBookings.filter((b) => {
        ...
        return mType.includes('invitation') || 
               mType.includes('print') ||
               sType.includes('invitation') || 
               sType.includes('print') ||
               pName.includes('invitation') || 
               pName.includes('print') ||
               b.invitationId || 
               b.invitationPackageId ||
               b.printerId ||
               (b.items && b.items.some(item => 
                 String(item.name || '').toLowerCase().includes('invitation') || 
                 String(item.name || '').toLowerCase().includes('print')
               ));
      });
      ...
      let packageNameMap = {};
      try {
        const pkgRes = await axios.get(`${API_BASE_URL}/api/invitations/provider/${providerId}`, config);
        const pkgs = pkgRes.data?.data || pkgRes.data || [];
        if (Array.isArray(pkgs)) {
          pkgs.forEach((p) => {
            packageNameMap[p._id] = {
              name: p.name || p.title || p.packageName || 'Invitation & Printing Service',
              price: p.price || p.pricing?.grandTotal || p.salePrice || 0
            };
          });
        }
      } catch (e) {
        console.warn('Invitation list fetch failed:', e);
      }
      ...
      invitationBookings.forEach((b) => {
        ...
        const pkgIdStr = typeof b.packageId === 'object' ? b.packageId?._id : (b.packageId || b.invitationId || b.invitationPackageId);
        ...
        if (!packageMap[pkgId]) {
          ...
          packageMap[pkgId] = {
            id: pkgId,
            name: nameStr || info?.name || 'Custom Invitation Service',
      ...
      setDashboardData({
        totalEarnings,
        totalOrders: invitationBookings.length, 
        totalIncome,
        monthlyOrders: monthlyOrdersCount,
        yearlyOrders: yearlyOrdersCount,
        bookings: invitationBookings,
        popularPackages,
        monthlyGrowth,
        monthlyIncomeGrowth,
        totalPackageRevenue
      ...
      console.error('Invitation Dashboard data fetch error:', err);
    } finally {
      setLoading(false);
    }
  }, []);
      ...
      <Grid size={12}>
        <Grid container spacing={gridSpacing}>
          <Grid size={{ xs: 12, sm: 6, md: 4 }}>
            <InvitationEarningCard isLoading={isLoading} totalEarnings={dashboardData.totalEarnings} />
          </Grid>
          <Grid size={{ xs: 12, sm: 6, md: 4 }}>
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <Grid container spacing={gridSpacing}>
              <Grid size={{ xs: 12, sm: 6, md: 12 }}>
                <InvitationIncomeDarkCard isLoading={isLoading} totalAmount={dashboardData.totalIncome} />
              </Grid>
              <Grid size={{ xs: 12, sm: 6, md: 12 }}>
